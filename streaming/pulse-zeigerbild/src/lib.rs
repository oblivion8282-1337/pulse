//! Das **Bildformat** der Zeigeruebertragung — die Laeufe, die Grenzen, die
//! Kennung.
//!
//! **Seit dem 2026-08-20 gemeinsam fuer den Windows-Sidecar und den Player.**
//! Vorher lag diese Datei wortgleich an zwei Stellen
//! (`streaming/win-hq-sidecar/src/zeigerbild.rs`,
//! `streaming/pulse-player/src/zeigerbild.rs`), gehalten von einem eigenen
//! Test (`zwillinge/tests/bitgleich.rs::zeigerbild_liegt_im_sidecar_wortgleich`,
//! seither durch einen erklaerenden Kommentar ersetzt). Beide Dateien sind
//! jetzt nur noch Re-Exports dieser Crate — gleiches Muster und gleicher Grund
//! wie bei `pulse-zeitbasis`: die beiden Enden muessen sich Byte fuer Byte
//! einig sein, und eine Beschreibung in zwei Fassungen ist eine Beschreibung,
//! die auseinanderlaeuft. Alles hier ist reine Rechnung — kein Windows, kein
//! Fenster, keine Sitzung —, deshalb haelt der Test unten **beide** Richtungen
//! fest, obwohl jede Seite nur eine braucht.
//!
//! ## Wozu ueberhaupt ein Bild
//!
//! Der gewoehnliche Weg uebertraegt einen **Namen** aus der CSS-Zeigerliste
//! (`remote_input/zeigerform.rs` → `app/zeigerform.rs`), und das ist der bessere
//! Weg, wo er traegt: ein paar Byte, gezeichnet wird der lokale Zeiger in der
//! Groesse und dem Thema des Steuernden. Er traegt aber nur die dreizehn
...
//! — die fielen alle auf den Standardpfeil, und der Steuernde saehe nichts von
...
//!
//! ## Warum eigene Laeufe und kein PNG
//!
//! Ein Zeiger ist zu grossen Teilen durchsichtig, oft mit nur zwei oder drei
//! echten Farben. Genau dafuer sind Wiederholungslaeufe gemacht: ein ueblicher
//! 32×32-Zeiger schrumpft von 4096 auf einige hundert Byte, und das Verfahren
//! sind vierzig Zeilen ohne Fremdcode. Ein PNG-Packer waere eine neue
//! Abhaengigkeit **auf beiden Seiten** — fuer ein Bild, das kleiner ist als sein
//! eigener Dateikopf.
//!
...
//! (4096 Byte → 5464 kodiert), ein 48×48 schon nicht mehr. Mit Laeufen passen
//! auch die grossen bequem — und wo sie es wider Erwarten nicht tun, faellt der
//! Sender wortlos auf den Namen zurueck, statt eine Nachricht zu schicken, die
//! unterwegs still verworfen wuerde.
//!
//! ## Der Aufbau
//!
...
//! andersherum — die Rueckrechnung macht der Sidecar, s. dort). Die Laeufe:
//!
...
//! | `0`, `128` | ungueltig | — |
//!
//! Ein Lauf fasst hoechstens 127 Punkte. Das kostet bei einer durchsichtigen
//! 32×32-Flaeche neun Steuerbytes statt einem und ist die Grenze wert: so bleibt
...
//! unterscheiden, koennte niemand mehr mit einem `diff` pruefen. Die Tests unten
//! nutzen ohnehin beide Richtungen; nur der Auslieferungsbau saehe sonst
//! Warnungen fuer Code, der auf der anderen Seite der Leitung gebraucht wird.
#![allow(dead_code)]
