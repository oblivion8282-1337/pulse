...
/// Bildkopf und Bild — beide beginnen mit dem unkomprimierten Bildkopf, und
/// nur dort steht, ob ein Bild ein Vollbild ist.
...
impl Obu<'_> {
    ...
    /// Steht in diesem OBU ein **echtes Vollbild**?
    ///
    /// Gelesen werden die ersten drei Bit des unkomprimierten Bildkopfs:
    /// `show_existing_frame` (1 Bit), dann `frame_type` (2 Bit), wobei 0 fuer
    /// `KEY_FRAME` steht.
    ///
    /// **Annahme, die fuer unsere Stroeme gilt:** `reduced_still_picture_header
    /// = 0`. Waere es 1, entfielen beide Felder — dann ist aber ohnehin jedes
    /// Bild ein Vollbild, und die Angabe waere nur zu vorsichtig, nie zu
    /// grosszuegig.
    fn ist_vollbild(&self) -> bool {
        if self.typ != OBU_BILDKOPF && self.typ != OBU_BILD {
            return false;
        }
        let Some(&b) = self.rumpf.first() else {
            return false;
        };
        (b & 0x80) == 0 && ((b >> 5) & 0b11) == 0
    }
...
/// `pub(crate)`, damit Entpacker (`av1_entpacken`) und Prüfwerkzeug (`whep`)
/// dieselbe Umsetzung benutzen statt eigener. Genau hier hat das `rtp`-Crate
/// seinen Fehler (s. Modulkopf) — zwei Fassungen davon im selben Haus wären die
/// schlechteste aller Antworten.
pub(crate) fn lies_leb128(daten: &[u8]) -> Result<(u32, usize)> {
    let mut wert: u64 = 0;
...
/// `pub(super)` aus demselben Grund wie [`lies_leb128`].
pub(super) fn schreibe_leb128(ziel: &mut Vec<u8>, mut wert: u32) {
    loop {
...
pub fn paketiere(daten: &[u8], mtu: usize) -> Result<Vec<Nutzlast>> {
    ...
    // **Ein Sequenzkopf allein ist noch keine neue Bildfolge.** Bis 2026-08-02
    // wurde `N` allein daraus abgeleitet, DASS ein Sequenzkopf im Zeitabschnitt
    // steht. Das ging so lange gut, wie der Encoder ihn ausschliesslich vor
    // Vollbildern schrieb — bei `av1_vulkan` und NVENC ist das so.
    //
    // `av1_amf` mit Auffrischung schreibt ihn dagegen an JEDEM GOP-Rand, auch
    // ohne Vollbild: gemessen 6 Sequenzkoepfe auf 1 Vollbild in 360 Bildern.
    //
    // Und die Markierung allein zu berichtigen genuegt nicht: ein Empfaenger
    // schliesst auch selbst von der blossen ANWESENHEIT eines Sequenzkopfs auf
    // ein Vollbild — Chromium tut das und versuchte an diesen Stellen
    // einzusteigen. Deshalb faellt ein Sequenzkopf ohne Vollbild ganz weg.
    //
    // Weglassen ist gefahrlos: der Kopf steht ohnehin in jedem echten Vollbild,
    // und ein spaet einsteigender Zuschauer bekommt eines nur auf Anforderung
    // (ohne die kaeme er bei Intra-Refresh gar nicht ins Bild, gemessen). Er
    // verliert hier also nichts, was er nicht ohnehin anfordern muss.
    let vollbild = obus.iter().any(Obu::ist_vollbild);
    if !vollbild {
    ...
    // Nach dem Block oben gilt: Sequenzkopf vorhanden ==> Vollbild vorhanden.
    // Die zweite Bedingung waere hier also tautologisch — sie steht trotzdem
    // da, weil `N` an das Vollbild gehoert und nicht an den Kopf. Faellt das
    // Weglassen je weg, ist diese Zeile weiterhin richtig.
    let mut neue_bildfolge = vollbild && obus.iter().any(|o| o.typ == OBU_SEQUENZKOPF);
    ...
    for (i, obu) in obus.iter().enumerate() {
        ...
        loop {
            ...
            pakete.push(baue(&obus, &stuecke, z, rest, &mut neue_bildfolge));
            stuecke.clear();
    ...
    if !stuecke.is_empty() {
        pakete.push(baue(&obus, &stuecke, z, false, &mut neue_bildfolge));
    }
...
///
/// `neue_bildfolge` wird beim ersten Paket geleert: `N` bedeutet "erstes
...
fn baue(
    ...
    z: bool,
    y: bool,
    neue_bildfolge: &mut bool,
) -> Nutzlast {
    let n = *neue_bildfolge && !z;
    if n {
        *neue_bildfolge = false;
    }
...
mod tests {
    ...
    /// Ein Bild-OBU mit frei gewaehltem `frame_type` — er steht in den ersten
    /// drei Bit des Rumpfs.
    ///
    /// **Warum es das braucht, obwohl `obu` schon Bilder baut:** dessen
    /// Fuellung beginnt bei 0, und das bedeutet zufaellig `KEY_FRAME`. Alle
    /// aelteren Tests, die `obu(6, …)` benutzen, pruefen deshalb den
    /// Vollbild-Fall — ohne es zu sagen. Wer die Fuellung von `obu` aendert,
    /// kippt sie stillschweigend in den Zwischenbild-Fall.
    fn bild_obu(frame_type: u8, rumpf_len: usize) -> Vec<u8> {
        let mut v = obu(OBU_BILD, rumpf_len);
        let ab = v.len() - rumpf_len;
        v[ab] = (frame_type & 0b11) << 5;
        v
    }

    /// **Der Fall, der am 2026-08-02 den Browser gestoert hat.** `av1_amf`
    /// schreibt den Sequenzkopf an jedem GOP-Rand, auch ohne Vollbild — gemessen
    /// sechs Sequenzkoepfe auf ein Vollbild. Wer daraus `N` ableitet, sagt dem
    /// Zuschauer fuenfmal zu Unrecht "hier kannst du einsteigen".
    #[test]
    fn sequenzkopf_ohne_vollbild_setzt_kein_n() {
        let mut tu = obu(OBU_ZEITTRENNER, 0);
        tu.extend(obu(OBU_SEQUENZKOPF, 12));
        tu.extend(bild_obu(1, 300)); // 1 = INTER_FRAME
        let p = paketiere(&tu, MTU).unwrap();
        assert!(!p.is_empty());
        for (i, q) in p.iter().enumerate() {
            assert_eq!(q.daten[0] & 0x08, 0, "N darf nirgends stehen (Paket {i})");
        }
        // Und der Kopf selbst darf gar nicht erst mitkommen: ein Empfaenger
        // schliesst sonst von seiner blossen Anwesenheit auf ein Vollbild.
        let wieder = setze_zusammen(&p);
        assert_eq!(wieder.len(), 1, "nur das Bild darf uebrig bleiben");
        assert_eq!(
            (wieder[0][0] >> 3) & 0x0F,
            OBU_BILD,
            "der Sequenzkopf haette wegfallen muessen"
        );
    }

    /// Und die Gegenprobe: mit echtem Vollbild MUSS `N` kommen. Ohne diesen Test
    /// waere die Berichtigung oben auch dann gruen, wenn sie `N` nie mehr setzt —
    /// und dann kaeme kein Zuschauer mehr ins Bild.
    #[test]
    fn sequenzkopf_mit_vollbild_setzt_n() {
        let mut tu = obu(OBU_ZEITTRENNER, 0);
        tu.extend(obu(OBU_SEQUENZKOPF, 12));
        tu.extend(bild_obu(0, 300)); // 0 = KEY_FRAME
        let p = paketiere(&tu, MTU).unwrap();
