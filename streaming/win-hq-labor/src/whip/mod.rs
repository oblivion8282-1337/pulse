...
//! [`pulse_win_hq_sidecar::keyframe::request_keyframe`].
//!
...
pub mod av1;
pub mod av1_entpacken;
mod av1_level;
/// Diagnose des Verbindungsaufbaus. Liegt bewusst DANEBEN und nicht in dieser
/// Datei: sie ist ansonsten eine wortgleiche Kopie aus dem Linux-Labor.
mod diagnose;
mod pacer;
...
///
/// **Warum das noetig ist** (gemessen 2026-08-02, Messakte
/// `ton-2026-08-02-windows-messstand.json`): `track_local_static_sample.rs`
/// rechnet `(dauer.as_secs_f64() * uhr) as u32` — und `as u32` **schneidet ab**.
/// Ein Bild bei 30/s dauert 1/30 s, das mal 90000 ergibt in f64
/// 2999,9999999999995, und daraus wird 2999 statt 3000. Ein Takt je Bild klingt
/// nach nichts und ist bei 30 Bildern je Sekunde **20 ms je Minute**: das Bild
/// laeuft dem Ton mit rund 1,3 Sekunden je Stunde davon, ohne dass irgendwo ein
/// Fehler auftaucht. Genau so gemessen — H.264 -21,9 und -20,5 ms/min, waehrend
/// AV1 (eigener Paketierer, ganzzahlige Rechnung) bei -0,5 und -0,0 lag.
///
/// **Eine halbe Takt-Zugabe, nicht eine ganze.** Damit landet das Abschneiden
/// sicher auf dem gewuenschten Wert, egal wie die f64-Darstellung fällt, und die
/// Dauer bleibt zugleich naeher am Soll als jede Aufrundung auf den naechsten
/// Takt. Der Rest — Bildabstand fuer den Taktgeber — nimmt weiter die echte
/// Dauer; diese hier ist ausschliesslich die Uebergabe an webrtc-rs.
...
pub struct WhipSender {
    ...
    http: reqwest::Client,
    /// Die Bilddauer, so zurechtgelegt, dass webrtc-rs daraus die richtige
    /// Zahl RTP-Takte macht (s. [`dauer_fuer_takte`]). **Nur** fuer die
    /// Uebergabe an `write_sample`; wer damit rechnet, rechnet falsch — die
    /// echte Bilddauer bekommt der Taktgeber beim Bau.
    bild_sample_dauer: Duration,
    /// Bildrate — auch in `Av1Zustand`, aber die Level-Korrektur braucht sie
    /// VOR dem Sperrbereich (s. `send`).
    fps: u32,
}
...
impl WhipSender {
    ...
    async fn connect_async(url: &str, cap: RTCRtpCodecCapability, fps: u32) -> Result<Self> {
        ...
        // Takte je Bild ganzzahlig, wie es der AV1-Paketierer auch tut — bei
        // 90000/fps ohne Rest (24/25/30/50/60...) ist das exakt, sonst der
        // naechstliegende Wert.
        ...
        // `PULSE_WHIP_LOOPBACK=1` sammelt zusaetzlich 127.0.0.1 als Kandidat.
        //
        // **Nur fuer Messungen gegen einen Server auf derselben Maschine**, und
        // dort nicht Bequemlichkeit, sondern Voraussetzung: ohne Loopback-
        // Kandidat laeuft der Verkehr ueber die LAN-Adresse und damit durch die
        // Windows-Firewall — die fuer ein frisch gebautes Binary ohne Zutun
        // eines Menschen eine Block-Regel bekommt (am 2026-08-02 genau so
        // passiert, `pulse-win-hq-labor.exe` Inbound Block). Auf Loopback
        // greift die Firewall nicht.
        //
        // Im echten Betrieb bleibt es aus: dort ist der Server nie auf
        // derselben Maschine, und ein Loopback-Kandidat im Angebot ist nur
        // eine Adresse, die die Gegenseite vergeblich probiert.
        let mut engine = SettingEngine::default();
        if pulse_win_hq_sidecar::env::flag("PULSE_WHIP_LOOPBACK") {
            engine.set_include_loopback_candidate(true);
            tracing::info!(target: "whip", "Loopback-Kandidaten an (PULSE_WHIP_LOOPBACK)");
        }
        ...
        diagnose::verbindung_mitschreiben(&pc);
        ...
        Ok(Self { track, audio, pc, resource_url, http, bild_sample_dauer, fps })
    }
    ...
    async fn negotiate(
        ...
        let vollstaendig =
            tokio::time::timeout(ICE_GATHERING_TIMEOUT, gathering.recv()).await.is_ok();
        let sdp = pc
            .local_description()
            .await
        ...
        diagnose::angebot_pruefen(&sdp, vollstaendig)?;
        ...
        let res = http
            ...
            // loggen.
            .map_err(|e| anyhow!("WHIP-Server nicht erreichbar: {}", pulse_win_hq_sidecar::redact::secrets(&e.to_string())))?;
    ...
    pub fn send(&self, data: &[u8]) -> Result<()> {
        match &self.track {
            ...
            Bildspur::Selbst { zustand, pacer, track } => {
                // **Das Level berichtigen, bevor paketiert wird** — sonst fällt
                // Chromiums Hardware-Decoder auf Software zurück und 10 Bit
                // bleibt schwarz (Begründung und Messung in [`av1_level`]).
                // Ausserhalb des Sperrbereichs: es ist ein Durchlauf über das
                // ganze Bild, und der gehört nicht dorthin.
                let berichtigt = av1_level::hebe_level_kopie(data, self.fps)?;
                let data = berichtigt.as_deref().unwrap_or(data);
                ...
                match pacer {
                    ...
                    // EIN Eintritt in die Laufzeit fuer das ganze Bild, nicht
                    // einer je Paket: ein Bild zerfaellt in 1 bis 12 Pakete
                    // (s. `pacer`), bei 60 Bildern je Sekunde waeren das bis zu
                    // 720 Eintritte statt 60. Jeder kostet Kontext und einen
                    // Parker — Arbeit, die nichts sendet.
                    None => runtime().block_on(async {
                        for paket in &pakete {
                            track.write_rtp(paket).await.context("write_rtp")?;
                        }
                        Ok(())
                    }),
                }
    ...
    /// nicht trifft: 5 ms mal 48000 ergibt in f64 knapp ueber 240 und wird
    /// richtig abgeschnitten. Das ist Zufall der Darstellung, nicht Absicht —
    /// bei einer anderen Paketlaenge (`PULSE_OPUS_FRAME_MS`) kann es
    /// andersherum ausgehen, und dann liefe der TON weg statt des Bildes. Ein
    /// gemessener Fehler an einer Stelle heisst, die gleiche Stelle ueberall
    /// zu schliessen.
...
mod tests {
    ...
    fn die_alte_rechnung_verliert_einen_takt() {
        ...
        let verlust_je_minute = 30.0 * 60.0 * (3000 - 2999) as f64 / 90_000.0 * 1000.0;
