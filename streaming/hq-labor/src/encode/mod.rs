...
// Eigene Kopien: nur diese drei aendert der WHIP-Weg.
pub mod audio;
pub mod mux_writer;
// Unveraendert gegenueber dem ausgelieferten Sidecar — aus seiner Bibliothek,
// damit die GPU-Importpfade nicht doppelt gepflegt werden muessen.
pub use pulse_linux_hq_sidecar::encode::{hw, nv_import, nv_p010, opts, raw_dump, va_import};
...
/// naechsten regulaeren Vollbild — bei zwei Sekunden Abstand also bis zu zwei
/// Sekunden (Hauptrepo, `verlust-2026-07-28-*.json`). Gemessen ist auch, dass
/// MediaMTX eine RTCP-Anforderung an einen WHIP-Sender weiterreicht; was fehlte,
/// war ein Sender, der sie beantworten kann.
///
/// Das ist diese Stelle. Ausgeloest wird sie heute ueber die Operation
/// `keyframe` auf der stdio-Schnittstelle — der Rueckkanal aus dem Netz kommt
/// erst mit einem eigenen WebRTC-Sendeweg. So laesst sich die WIRKUNG messen,
/// bevor der Transport gebaut wird.
///
/// Nebennutzen unabhaengig davon: ein neu dazukommender Zuschauer wartet heute
/// bis zu zwei Sekunden auf sein erstes Bild.
...
/// Sender gegen Paketverlust hat. Am 2026-07-28 gemessen (Hauptrepo,
/// `verlust-2026-07-28-browser-gegen-nativ.json`): Der native Player wartet nach
/// jeder Luecke auf den naechsten Einstiegspunkt, und bei zwei Sekunden Abstand
/// wird er dadurch unberechenbar — drei identische Laeufe unter 1 % Verlust
/// ergaben Mediane von 38, 190 und 369 ms, mit Ausschlaegen bis 539. Chromium
/// wartet nicht, sondern dekodiert weiter, und liegt deshalb unter Verlust vorn.
///
...
/// deshalb erst ein Schalter und keine neue Zahl.
...
/// Encoder aufbauen und oeffnen — ohne jeden Bezug zu einem Ausgabe-Kontext.
///
/// **Warum herausgeloest.** Der eigene WHIP-Sendeweg braucht denselben Encoder,
/// aber keinen ffmpeg-Ausgang: dort gibt es weder Container noch Stream. Ein
/// Duplikat waere hier besonders teuer — in dieser Einrichtung stecken mehrere
/// gemessene Entscheidungen (Vorlauf aus, kein B-Bild, Farbsignalisierung nur
/// im 10-bit-Pfad, `preset=p2`), und zwei Fassungen davon liefen auseinander,
/// ohne dass es jemand bemerkt.
///
/// `global_header` kommt von aussen, weil nur der Aufrufer weiss, ob ein
/// Container die Parametersaetze getrennt erwartet (FLV, Matroska) oder ob sie
/// im Strom mitlaufen muessen (RTP — dort waere ein globaler Kopf falsch, der
/// Empfaenger bekaeme nie SPS/PPS).
///
/// # Safety
///
/// `frames_ctx` muss ein gueltiger `AVHWFramesContext`-`AVBufferRef` sein, der
/// zu `hw_pixel` passt, und mindestens bis zum Oeffnen leben.
