...
use pulse_hq_labor::{dispatch, events, logging};
...
fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(
        ...
        pid = std::process::id(),
        "pulse-hq-labor startet (Messstand, nicht der ausgelieferte Sidecar)"
    );
    ...
    let _ = pulse_hq_labor::stream_controller::StreamController::singleton().stop();
