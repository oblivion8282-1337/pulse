...
//! becomes `{"ok": true, ...map}`.
//!
//! No `exit_after` flag (unlike the Windows sidecar): macOS keeps the sidecar
//! process warm across streams — see `main.rs` and the crate README.
...
fn dispatch(req: Request) -> Response {
    ...
    let result: anyhow::Result<Map<String, Value>> = match req.op.as_str() {
        ...
        "direct_offer" => ops::direct_offer::handle(req.params),
        "direct_stop" => ops::direct_stop::handle(req.params),
        "state" => ops::state::handle(req.params),
        "keyframe" => ops::keyframe::handle(req.params),
        "ablage" => ops::ablage::handle(req.params),
