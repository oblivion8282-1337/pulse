...
//!
//! Lets worker threads (the future StreamController, FPS tracker, etc.) emit
//! events without knowing the `main.rs` write logic, via an
...
//! `streaming/win-hq-sidecar/src/events.rs` — a single writer thread resolves
//! the race between response writers and event writers on stdout.
