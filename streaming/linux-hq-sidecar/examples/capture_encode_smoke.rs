...
//!
//! ```text
//! cargo run --release --example capture_encode_smoke -- /tmp/pulse_capture.mp4 [codec] [fps] [frames] [bit_depth]
//! ffprobe -v error -show_streams /tmp/pulse_capture.mp4
//! ```
//! `bit_depth` = 8 (Default) oder 10. 10 bit geht nur mit `av1` — so lässt
//! sich der 10-bit-Pfad ohne RTMPS/MediaMTX prüfen: `pix_fmt` der Ausgabe
//! muss dann `yuv420p10le` sein.
...
use pulse_linux_hq_sidecar::encode::{EncoderConfig, VideoEncoder, hw, nv_import, nv_import::NvDmabufImporter};
...
fn main() -> anyhow::Result<()> {
    ...
    let ten_bit = std::env::args().nth(5).as_deref() == Some("10");
    if ten_bit && codec != "av1" {
        anyhow::bail!("10 bit nur mit codec=av1 (H.264-10-bit dekodiert kein Browser)");
    }
    let staging = if ten_bit {
    ...
    // CUDA-Pool mit dem sw_format des Staging-Formats (NVENC nimmt RGB
    // direkt; Primary-Context teilt sich FFmpeg mit dem Importer).
    let hw_ctx =
        hw::HwContext::create(hw::HwDeviceKind::Cuda, None, w, h, staging.av_pix_fmt())?;
