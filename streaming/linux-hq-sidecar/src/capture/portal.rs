...
pub struct PortalSession {
    ...
    /// nächsten Start). Wird hinter `PULSE_PORTAL_REUSE=1` persistiert (s.
    /// `store_restore_token`) — ohne den Schalter bleibt es nur dieser
    /// Rückgabewert, ungespeichert.
...
/// Ablage für das Restore-Token des Portals.
///
/// Eigene Datei statt Umweg über die Pulse-App: das Token gehört zu DIESEM
/// Prozess auf DIESEM Rechner (das Portal bindet es an die App-Identität) und
/// hat im geräteübergreifenden Einstellungs-Store nichts zu suchen.
fn restore_token_path() -> std::path::PathBuf {
    ...
    base.join("pulse/portal-restore-token")
}

/// Darf der Dialog übersprungen werden?
///
/// **Standardmäßig nein.** Unter Wayland IST der Portal-Dialog die
/// Quellenauswahl — überspringt man ihn still, hängt der Nutzer für immer an
/// der zuerst gewählten Quelle, ohne zu sehen warum. Deshalb ist das
/// Wiederverwenden ausdrücklich zu verlangen (`PULSE_PORTAL_REUSE=1`), heute
/// für automatisierte Messungen ohne Klick. Ein späterer Nutzer-Schalter
/// („diese Quelle merken") gehört an dieselbe Stelle.
fn reuse_requested() -> bool {
    matches!(std::env::var("PULSE_PORTAL_REUSE").as_deref(), Ok("1"))
}
...
/// Token wegschreiben. Fehler sind nicht schlimm — dann kommt beim nächsten
/// Start eben wieder der Dialog, und das ist der harmlose Ausgang.
fn store_restore_token(token: &str) {
    if !reuse_requested() || token.is_empty() {
        return;
    }
    ...
    if std::fs::write(&path, token).is_ok() {
        // Das Token erlaubt das Wiederaufnehmen einer Bildschirmaufnahme —
        // nichts, was andere Nutzer des Rechners lesen sollen.
        #[cfg(unix)]
        {
...
async fn negotiate(
    ...
    sc.select_sources(
        ...
        // Nur persistieren, wenn wir das Token auch einlösen: sonst legte das
        // Portal bei jedem Start einen Permission-Eintrag an, der nie
        // wiederverwendet noch zurückgenommen wird — die sammeln sich im
        // Portal-Store an.
        if reuse_requested() {
    ...
    // Das Portal gibt bei jeder Verhandlung ein FRISCHES Token aus und entwertet
    // das alte — wegschreiben ist also Pflicht, nicht Kosmetik: ohne das wäre
    // schon der zweite Start wieder ein Dialog.
    if let Some(t) = restore_token.as_deref() {
        store_restore_token(t);
    }
