...
//!
//! [`super::Zustand::zeiger`] hält den Punkt, den **wir** zuletzt gesetzt haben
...
//!   bewegt hat (`SendInput` arbeitet die Warteschlange in Reihenfolge ab).
//!
...
//! auflegt. Genau diese Beschleunigung ist von hier aus aber nicht
...
use super::rahmen::InputFrame;
use super::{Zustand, injektion, zuordnung};
...
pub(super) fn einspielen(
    z: &mut Zustand,
    rechteck: Option<RECT>,
    frame: InputFrame,
) -> Result<(), String> {
    match frame {
...
fn bewegen(z: &mut Zustand, punkt: Option<(i32, i32)>) {
    if let Some(p) = punkt {
        absolut_setzen(p);
    }
    z.zeiger = punkt;
...
fn tat_ort(z: &Zustand, rechteck: Option<RECT>) -> Option<(i32, i32)> {
    let rect = rechteck?;
    let p = z.zeiger?;
    (zuordnung::klemmen(p.0, p.1, &rect) == Some(p)).then_some(p)
}

/// Einen physischen Bildschirmpunkt als absolute Bewegung abfeuern.
fn absolut_setzen((px, py): (i32, i32)) {
...
mod tests {
    use super::*;
    use crate::remote_input::injektion::pruefspur::{self, Ereignis};
    ...
    const QUELLE: RECT = RECT { left: 100, top: 200, right: 1100, bottom: 800 };

    fn zustand(zeiger: Option<(i32, i32)>) -> Zustand {
        let _ = pruefspur::nimm();
        Zustand { begruesst: true, zeiger, ..Zustand::default() }
    }

    fn maus_ereignisse(spur: &[Ereignis]) -> usize {
    ...
    fn relative_bewegung_verlaesst_das_rechteck_nie() {
        for start in [None, Some((100, 200)), Some((1099, 799)), Some((600, 500))] {
            ...
            for (dx, dy) in [
                ...
                assert!(
                    (QUELLE.left..QUELLE.right).contains(&px)
                        && (QUELLE.top..QUELLE.bottom).contains(&py),
    ...
    fn relative_bewegung_wird_geklemmt_gesetzt() {
        ...
        einspielen(&mut z, Some(QUELLE), InputFrame::MouseMoveRel { dx: -32768, dy: -32768 })
            .unwrap();
        assert_eq!(z.zeiger, Some((QUELLE.left, QUELLE.top)));
        let vd = zuordnung::virtueller_desktop();
        let (nx, ny) = zuordnung::punkt_auf_absolut(QUELLE.left, QUELLE.top, &vd);
        let spur = pruefspur::nimm();
        assert_eq!(spur.len(), 1, "{spur:?}");
        match spur[0] {
            Ereignis::Maus { dx, dy, flags, .. } => {
                assert_eq!((dx, dy), (nx, ny));
                assert_eq!(flags & MOUSEEVENTF_ABSOLUTE.0, MOUSEEVENTF_ABSOLUTE.0);
            }
    ...
    fn knopf_feuert_nicht_ohne_gueltige_lage() {
        ...
        einspielen(&mut z, Some(QUELLE), InputFrame::MouseButton { btn: 0, down: true }).unwrap();
        assert!(pruefspur::nimm().is_empty(), "ohne Lage darf nichts feuern");
    ...
    fn knopf_feuert_nicht_ausserhalb_des_rechtecks() {
        ...
        let mut z = zustand(Some((600, 500)));
        einspielen(&mut z, None, InputFrame::MouseButton { btn: 0, down: true }).unwrap();
        assert!(pruefspur::nimm().is_empty());
    }
    ...
    fn verworfene_bewegung_entwertet_die_lage() {
        let mut z = zustand(Some((600, 500)));
        einspielen(&mut z, None, InputFrame::MouseMoveAbs { x: 32767, y: 32767 }).unwrap();
        assert_eq!(z.zeiger, None);
        let _ = pruefspur::nimm();
        einspielen(&mut z, Some(QUELLE), InputFrame::MouseButton { btn: 0, down: true }).unwrap();
        assert!(pruefspur::nimm().is_empty(), "Klick nach verworfener Bewegung");
    }
    ...
    fn knopf_feuert_mit_gueltiger_lage() {
        ...
        einspielen(&mut z, Some(QUELLE), InputFrame::MouseButton { btn: 0, down: true }).unwrap();
        let spur = pruefspur::nimm();
        assert_eq!(maus_ereignisse(&spur), 2, "Lage behaupten + Knopf: {spur:?}");
    ...
    fn loslassen_geht_auch_ohne_lage_durch() {
        ...
        einspielen(&mut z, None, InputFrame::MouseButton { btn: 1, down: false }).unwrap();
        assert_eq!(maus_ereignisse(&pruefspur::nimm()), 1, "das Hoch-Ereignis muss raus");
    ...
    fn unbekannter_knopf_ist_fail_closed() {
        let mut z = zustand(None);
        assert!(
            einspielen(&mut z, None, InputFrame::MouseButton { btn: 9, down: true }).is_err()
        );
    }
    ...
    fn rad_feuert_nicht_ohne_gueltige_lage() {
        let mut z = zustand(None);
        einspielen(&mut z, Some(QUELLE), InputFrame::MouseWheel { dv: 120, dh: 0 }).unwrap();
        assert!(pruefspur::nimm().is_empty());

        let mut z = zustand(Some((600, 500)));
        einspielen(&mut z, Some(QUELLE), InputFrame::MouseWheel { dv: 120, dh: -120 }).unwrap();
        assert_eq!(
            maus_ereignisse(&pruefspur::nimm()),
            3,
            "Lage behaupten + senkrecht + waagerecht"
        );
    }
    ...
    fn missgeformter_scancode_ist_fail_closed() {
        ...
        assert!(pruefspur::nimm().is_empty(), "und schon gar nichts injiziert");
    }
    ...
    fn taste_braucht_keine_zeigerlage() {
        let mut z = zustand(None);
        einspielen(&mut z, None, InputFrame::Key { scan: 0x11, down: true }).unwrap();
        assert_eq!(
            pruefspur::nimm(),
            vec![Ereignis::Taste { scan: 0x11, hoch: false }]
        );
