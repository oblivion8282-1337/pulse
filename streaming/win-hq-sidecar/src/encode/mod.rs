...
pub fn log_encoder_open(
    ...
    // Zwei Zustaende. Der zweite ist seit dem 2026-08-19 der Regelfall auf
    // `h264_amf`: der Encoder liefert den bestellten Vollbild-Takt nicht von
    // sich aus, also kommen die Vollbilder aus dem Selbsttakt. Das gehoert in
    // die Zeile — sie ist die Stelle, an der man nachsieht, was wirklich lief.
    //
    // Die Zeile sagt dabei nur, was sie belegen kann: dass die Vollbilder von
    // aussen getaktet werden. **Nicht** behauptet wird, dass daneben eine
    // rollende Auffrischung laeuft — die Begruendung samt der drei
    // gescheiterten Nachweisversuche steht an
    // [`auffrischung::braucht_selbsttakt`].
    let auffrischung = if auffrischung::braucht_selbsttakt(codec_name) {
