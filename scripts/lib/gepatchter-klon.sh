# Gemeinsamer Helfer: einen fremden Quellbaum auf einen festen Stand holen —
# wahlweise mit unseren Patches darauf — wiederholbar.
#
...
#     flacher_klon    <repo-url> <ref> <zielverzeichnis> [pruef-commit]
#
# Benutzt von `streaming/pulse-player/scripts/bootstrap-webrtc.sh` (webrtc-rs,
# mit Patches) und `streaming/ffmpeg-bau/bootstrap-ffmpeg.sh` (FFmpeg, ohne).
# Beide hatten das vorher eigenstaendig ausgeschrieben; die zwei Erkenntnisse
# unten waren damit doppelt gepflegt und drohten auseinanderzulaufen.

# Holt `$ref` aus `$repo` nach `$ziel`. Mehrfach aufrufbar: ein zweiter Lauf
# setzt erst auf den Ausgangsstand zurueck.
#
...
flacher_klon() {
    local repo="$1" ref="$2" ziel="$3" pruef_commit="${4:-}"
    ...
    if [ -d "$ziel/.git" ]; then
        ...
        # Dateien stehen, und genau die sind bei einem gepatchten Baum ja die
        # Patches. Der Rueckfall wirkte deshalb nie; mit einem einzigen Patch
        # fiel es nur nicht auf, weil `apply --check` danach am schon
        # gepatchten Baum scheiterte und das Skript mit "wurde die
        # Abhaengigkeit angehoben?" abbrach — einer Meldung, die in die
    ...
    _zeitstempel_einfrieren "$ziel"
}

# Wie [`flacher_klon`], danach alle `*.patch` aus `$patch_dir` in
...
gepatchter_klon() {
    ...
    shopt -u nullglob
    # Leer heisst hier IMMER ein Versehen (vertippter Pfad, verschobenes
    # Verzeichnis): wer nichts anzuwenden hat, ruft `flacher_klon`. Ohne diese
    # Wache saehe ein Lauf erfolgreich aus und lieferte einen ungepatchten
    # Baum — genau der Fehler, gegen den die ganze Datei gebaut ist.
    if [ ${#patch_dateien[@]} -eq 0 ]; then
        echo "Keine Patches in $patch_dir" >&2
        return 1
    fi

    flacher_klon "$repo" "$ref" "$ziel" "$pruef_commit" || return 1
...
_zeitstempel_einfrieren() {
    find "$1" -path "$1/.git" -prune -o -exec touch -h -d '2020-01-01T00:00:00Z' {} + 2>/dev/null || true
}
