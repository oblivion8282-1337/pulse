...
# ---- voice-move (MOVE_MEMBERS in source + target) ---------------------


@pytest.mark.asyncio
async def test_move_requires_move_members(app_with_redis, auth_signer, monkeypatch):
    client, _redis = app_with_redis
    monkeypatch.setattr(
        voice_routes.get_settings(), "chat_gateway_url", "http://chat-gateway.test"
    )
    # Only MUTE_MEMBERS — not enough to move.
    monkeypatch.setattr(
        voice_routes, "_chat_gateway_request",
        _make_voice_channel_mock(_PERM_CONNECT | _PERM_MUTE_MEMBERS),
    )
    access = auth_signer.issue_access(42, "alice")
    r = await client.post(
        "/channels/987654321/members/99/voice-move",
        json={"target_channel_id": "111222333"},
        headers=auth(access),
    )
    assert r.status_code == 403
...
@pytest.mark.asyncio
async def test_move_rejects_self(app_with_redis, auth_signer, monkeypatch):
    client, _redis = app_with_redis
    monkeypatch.setattr(
        voice_routes.get_settings(), "chat_gateway_url", "http://chat-gateway.test"
    )
    ...
    access = auth_signer.issue_access(42, "alice")
    r = await client.post(
        "/channels/987654321/members/42/voice-move",
        json={"target_channel_id": "111222333"},
        headers=auth(access),
    )
    assert r.status_code == 400
...
@pytest.mark.asyncio
async def test_move_rejects_same_channel(app_with_redis, auth_signer, monkeypatch):
    client, _redis = app_with_redis
    monkeypatch.setattr(
        voice_routes.get_settings(), "chat_gateway_url", "http://chat-gateway.test"
    )
    ...
    r = await client.post(
        "/channels/987654321/members/99/voice-move",
        json={"target_channel_id": "987654321"},
        headers=auth(access),
    )
    assert r.status_code == 400
...
@pytest.mark.asyncio
async def test_move_publishes_event(app_with_redis, auth_signer, monkeypatch):
    client, redis = app_with_redis
    monkeypatch.setattr(
        voice_routes.get_settings(), "chat_gateway_url", "http://chat-gateway.test"
    )
