...
class RegisteredInstance(Base):
    ...
    __table_args__ = (
        Index("uq_registered_instances_hostname", "hostname", unique=True),
        Index("uq_registered_instances_client_id", "client_id", unique=True),
...
class InstanceApplication(Base):
    ...
    approved_instance_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("registered_instances.id"), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
