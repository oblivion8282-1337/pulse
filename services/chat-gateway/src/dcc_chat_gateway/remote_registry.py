...
*ask* to drive another member's machine. Video + input run **P2P** directly
between the two browsers (webrtc-rs ↔ browser); the chat-gateway is only the
**signaling relay + consent gate** and never sits in the latency path. This
mixin holds the tiny amount of session state that relay needs: which two
sockets belong to a session and whether the host has consented yet.
...
SDP/ICE across pods). Exactly **one** session per host at a time.
