...
Each HQ stream (channel × user) has its own short-lived chat — separate from
the channel's regular text chat so viewers can react without bothering members
who aren't watching. Lifetime is tied to the stream, not the channel:

  * Storage: Redis List ``stream:chat:channel-<cid>-<uid>``, capped at
    ``MAX_HISTORY`` entries via ``LTRIM``, ``EXPIRE 6h`` to match the rest of
    the ``stream:*`` keyspace. **No DEL on stream-end** — the MediaMTX
    poll-list lags 1–3s and short publisher drops would otherwise nuke the
    chat mid-stream; let TTL do the cleanup.
  * Pub/sub: fan-out reuses the existing ``chat:channel:<cid>`` channel with
    an ``op: "stream_chat_message"`` envelope — gets per-channel filtering
    via ``_subs`` for free (chat is high-frequency, a global fan-out across
    ``_connections`` like ``stream_state`` would be too expensive).

Routes:
  * ``POST /channels/{cid}/streams/{uid}/chat`` — append. Member of the
    channel's guild + channel must be voice + ``stream:active:*`` must exist
    (else 410 Gone). Shared rate-limit bucket with channel-chat (``message``).
...
# Mirror of media-svc's ACTIVE_KEY. Duplicated rather than imported because the
# two services share no code on purpose (see CLAUDE.md / streamkeys.py note).
