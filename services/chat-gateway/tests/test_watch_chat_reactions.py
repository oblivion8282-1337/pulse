...
Covers ``PUT /channels/{cid}/watch-party/chat/{mid}/reactions/{emoji}/@me``:
...
async def _start_party(redis: Redis, channel_id: str, host_uid: int) -> str:
    """Seed an active watch-party state key so the gate passes."""
    key = f"watch:channel-{channel_id}"
    await redis.set(
        key,
        json.dumps({"host_user_id": str(host_uid), "position": 0.0, "is_playing": True}),
    )
    return key


async def _post_msg(client, channel_id: str, token: str, content: str) -> str:
    r = await client.post(
        f"/channels/{channel_id}/watch-party/chat",
...
def _react_url(cid: str, mid: str, emoji: str) -> str:
    return f"/channels/{cid}/watch-party/chat/{mid}/reactions/{emoji}/@me"


async def _cleanup(redis: Redis, channel_id: str) -> None:
    await redis.delete(
        f"watch:channel-{channel_id}",
        f"watch:chat:channel-{channel_id}",
        f"watch:chat:react:channel-{channel_id}",
    )
...
async def test_toggle_adds_then_removes(client, _auth_signer, redis):
    ...
    await _start_party(redis, cid, uid)
    try:
        mid = await _post_msg(client, cid, token, "hi")

        # First toggle → added, count 1, me True.
        r = await client.put(_react_url(cid, mid, "🔥"), headers=_auth(token))
        assert r.status_code == 200, r.text
        body = r.json()
        assert body == {"emoji": "🔥", "count": 1, "me": True}

        # Stored in the hash.
        raw = await redis.hget(f"watch:chat:react:channel-{cid}", mid)
        ...
        assert await redis.hget(f"watch:chat:react:channel-{cid}", mid) is None
    finally:
...
async def test_get_backfill_includes_reactions(client, _auth_signer, redis):
    ...
    await _start_party(redis, cid, uid)
    try:
        mid = await _post_msg(client, cid, token, "react to me")
        await client.put(_react_url(cid, mid, "👍"), headers=_auth(token))

        r = await client.get(
            f"/channels/{cid}/watch-party/chat", headers=_auth(token)
        )
        assert r.status_code == 200, r.text
        msgs = r.json()
        assert len(msgs) == 1
...
async def test_two_users_stack_and_me_flag_per_user(client, _auth_signer, redis):
    ...
    await _start_party(redis, cid, owner_uid)
    try:
        mid = await _post_msg(client, cid, owner_token, "double react")
        await client.put(_react_url(cid, mid, "🎉"), headers=_auth(owner_token))
        r = await client.put(_react_url(cid, mid, "🎉"), headers=_auth(member_token))
        assert r.status_code == 200, r.text
        ...
        owner_view = (
            await client.get(
                f"/channels/{cid}/watch-party/chat", headers=_auth(owner_token)
            )
        ).json()
        assert owner_view[0]["reactions"] == [{"emoji": "🎉", "count": 2, "me": True}]
        member_view = (
            await client.get(
                f"/channels/{cid}/watch-party/chat", headers=_auth(member_token)
            )
        ).json()
...
async def test_toggle_410_without_active_party(client, _auth_signer, redis):
    ...
    _, cid = await _setup_voice_channel(client, token)
    # No party seeded → 410.
    r = await client.put(_react_url(cid, "123", "🔥"), headers=_auth(token))
...
async def test_toggle_403_non_member(client, _auth_signer, redis):
    ...
    try:
        r = await client.put(
            _react_url(cid, "123", "🔥"), headers=_auth(outsider_token)
        )
...
async def test_toggle_400_empty_emoji(client, _auth_signer, redis):
    ...
    try:
        ...
        r = await client.put(_react_url(cid, "123", "%20"), headers=_auth(token))
    ...
    def _run():
        with TestClient(ws_app) as tc:
            ...
            try:
                r.set(
                    f"watch:channel-{cid}",
                    json.dumps({"host_user_id": str(uid), "is_playing": True}),
                )
                ...
                r.delete(
                    f"watch:channel-{cid}",
                    f"watch:chat:channel-{cid}",
                    f"watch:chat:react:channel-{cid}",
                )
                r.close()
