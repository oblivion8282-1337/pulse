...
{#if sichtbar}
  <button
    ...
    class="border-border hover:bg-bg-hover text-text-bright mt-3 flex min-h-12 w-full items-center gap-3 rounded-[14px] border border-dashed px-3 py-2.5 text-left text-sm font-medium transition-colors"
    data-testid="rooms-open-self-host"
  >
    <ServerIcon class="text-text-muted size-5 shrink-0" />
    <span class="min-w-0 flex-1 truncate">{m.self_host_entry_label()}</span>
    {#if hinweis}
      <span
