...
<script lang="ts">
  ...
  async function reload() {
    listLoading = true;
    try {
      applications = await instancesApi.listMyApplications('all');
    } catch {
  ...
  async function submit() {
    formError = null;
    if (!hostname.trim()) { formError = 'Hostname ist erforderlich.'; return; }
    submitting = true;
    try {
      ...
      toast.success('Antrag eingereicht. Du bekommst Bescheid wenn dein Antrag geprüft wurde.');
      hostname = '';
      notes = '';
      await reload();
    } catch (e) {
...
<div class="flex flex-col gap-5" data-testid="self-host-application">
  <div class="flex items-start gap-3">
    ...
    <div>
      <h3 class="text-text-bright text-sm font-semibold">Self-Hoster werden</h3>
      <p class="text-text-muted text-xs mt-0.5">
        Beantrage deine eigene Pulse-Instanz. Nach Admin-Prüfung erhältst du einmalig dein
        Client-Secret.
      </p>
    </div>
  </div>
  ...
  <form onsubmit={(e) => { e.preventDefault(); void submit(); }}
    ...
    <div class="flex flex-col gap-1">
      <label class="text-text-bright text-xs font-medium" for="sha-hostname">
        Hostname <span class="text-text-muted font-normal">(FQDN, z.B. pulse.example.org)</span>
      </label>
      <input
        id="sha-hostname"
        type="text"
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="flex flex-col gap-1">
        <label class="text-text-bright text-xs font-medium" for="sha-purpose">Verwendungszweck</label>
        <select
          id="sha-purpose"
          bind:value={purpose}
      ...
      <div class="flex flex-col gap-1">
        <label class="text-text-bright text-xs font-medium" for="sha-users">Erwartete Nutzer</label>
        <input
          id="sha-users"
          type="number"
          min="1"
    ...
    <div class="flex flex-col gap-1">
      <label class="text-text-bright text-xs font-medium" for="sha-email">Kontakt-E-Mail</label>
      <input
        id="sha-email"
        type="email"
    ...
    <div class="flex flex-col gap-1">
      <label class="text-text-bright text-xs font-medium" for="sha-notes">
        Anmerkungen <span class="text-text-muted font-normal">(optional)</span>
      </label>
      <textarea
        id="sha-notes"
        bind:value={notes}
        rows="2"
        maxlength="2000"
        placeholder="Wofür nutzt du die Instanz? Besonderheiten?"
  ...
  {#if !listLoading && applications.length > 0}
    <div class="flex flex-col gap-2">
      <h4 class="text-text-muted text-xs font-semibold uppercase tracking-wide">Meine Anträge</h4>
