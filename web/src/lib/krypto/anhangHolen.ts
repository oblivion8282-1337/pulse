 ...
 *
 * **Die wichtigste Regel dieses Moduls steht nicht hier, sondern in
 * `empfangen.ts`: geholt wird VOR der Quittung.** Das Abrufrecht haengt an
 * der eigenen offenen Zustellung (`postfach_anhaenge.py::darf_anhang_abrufen`),
 * und der Klumpen selbst faellt, sobald die letzte Zustellung quittiert ist
 * (`postfach_pflege.py::sweep_verwaiste_anhaenge`). Wer erst quittiert und
 * dann laedt, laedt ins Leere — und zwar endgueltig, es gibt keine zweite
 * Kopie.
 *
 * Daraus folgt der zweite Teil: was geholt wurde, wird LOKAL abgelegt
...
export async function anhangSichern(kanalId: string, anhang: Attachment): Promise<void> {
  ...
  const daten = await klumpenOeffnen(
    adressen.url,
    anhang.schluessel,
 ...
 *
 * `null`, wenn es sie weder lokal noch auf dem Server (mehr) gibt: der
 * Regelfall dafuer ist ein Anhang, dessen Abholung seinerzeit fehlschlug und
