...
export class AblageVerbindungsStore {
  ...
  private async patch(id: string, aenderung: Partial<AblageVerbindung>): Promise<void> {
    ...
    // Entproxyt wird an der Grenze (`verbindungenDb.ts::entproxyt`), nicht
    // hier: `bestehend` kommt aus einem `$state`-Feld, und IndexedDB wirft
    // an einem Proxy `DataCloneError`. Am 2026-09-01 hat das dreimal
    // zugeschlagen — u. a. blieb `fuerKanal` stumm auf `null`, waehrend
    // serverseitig alles korrekt gespeichert war.
