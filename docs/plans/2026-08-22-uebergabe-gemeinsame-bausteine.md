...
- [x] **Zeigerformen über die Fernsteuerung** — vom Nutzer an der Windows-Maschine geprüft und als funktionierend bestätigt. Damit ist der Weg durch `pulse-zeigerbild` im Betrieb belegt, und der ist sonst nirgends geprüft. *(Nicht protokolliert ist, ob ein **selbstgemalter** Zeiger dabei war — Resolve, Premiere, Blender. Der geht als Bild über die Leitung statt als Name, ist also der engere der beiden Pfade. Wer ohnehin an einer Windows-Maschine sitzt, nimmt ihn im Vorbeigehen mit.)*
