...
- **Gotchas**: Secrets nur server-seitig in `.env` + `secrets/jwt_*.pem` — **PEM `chmod 0644`** (Container uid 10001). Avatar-Volume bei Fresh-Deploy `chown 10001:10001` (sonst Upload-500). UFW: `7880`/`9997` nur vom Docker-Bridge (`ufw allow from 10.0.0.0/8`) — sonst blockt `INPUT DROP` den Bridge→Host-Weg. Migrate-Container laufen seit dem Cron-Updater (2026-06-11) automatisch beim Deploy mit (der frühere manuelle `up -d migrate-*`-Schritt entfällt; Memory `watchtower-skips-migrate-containers` für die Historie/Diagnose).
