...
- **Erreichbarkeits-Diagnose (seit 2026-08-25)** — ein Self-Host muss sieben Glieder hintereinander bestehen (DNS, TCP/443, Zertifikat, fremder Proxy, CORS, **WebSocket-Upgrade**, UDP-Medienports); bis dahin fasste jede Stelle alles davon zu einem „nicht erreichbar" zusammen. **Seit 2026-08-27 kommt ein achtes Glied dazu, das keine Erreichbarkeitsfrage stellt: „erkennt dieser Server dich als Betreiber?"** (`selfhost_probe_betreiber.py` ↔ chat-gateway `routes/owner_check.py`). **Voll-Doku: `docs/selfhost-erreichbarkeit.md`.** Was man ausserhalb wissen muss:
