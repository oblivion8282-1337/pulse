...
**Nativer HQ-Player** (`streaming/pulse-player/`, Rust, neu 2026-07-26) — der einzige Sidecar, der **empfängt** statt zu senden: WHEP → Jitter-Puffer → Depacketisierung → FFmpeg-Decode → eigenes wgpu-Fenster. Gleiches stdio-JSON-RPC (`open`/`close`/`set_option`/`stats`). **Rein additiv**: Browser und jede Installation ohne das Binary bleiben auf `WhepPlayer.svelte` im `<video>`; Opt-in `useNativePlayer` (Default false, **ohne UI** — nur `<userData>/pulse-stream.json`), Rückfall bei `state: "failed"`. Die nicht-offensichtlichen Stücke:
- **Bedienung sitzt IM Fenster** (2026-07-26, `src/overlay.rs` mit **egui 0.35** — neue Abhängigkeit, MIT/Apache): Lautstärke/Stumm, Vollbild, Statistik-Feld. Zwei Dinge daran sind nicht offensichtlich: (1) **egui muss mit wgpu/winit im Gleichschritt wandern** — `egui-wgpu 0.35` verlangt `wgpu ^29.0` + `winit ^0.30.13`; wer wgpu anhebt, ohne egui mitzuziehen, bekommt keinen Build. (2) Das Fenster öffnet **ohne Aktivierung** (`with_active(false)`), weil Pulses Tastenkürzel am Fenster der Web-App hängen (`VoiceChannelView.svelte`, `<svelte:window>` + `onblur` gibt PTT frei) — ein aktives Player-Fenster nimmt sie tot. Wer im Fenster bedient, nimmt den Fokus zwangsläufig; Klick zurück in Pulse stellt ihn her. Overlay blendet nach 3 s Ruhe aus und wird dann NICHT aufgebaut (`ControlFlow::Wait` — sonst 60 Weckrufe/s für nichts). Lautstärke-Änderung im Fenster kommt als `player:option`-Ereignis zurück, damit die App sie je Streamer behält; der Weg zurück ins Fenster fehlt dort absichtlich (Schleife).
- **Der Ton gehört GENAU EINEM Weg** (2026-07-26): das Fenster dekodiert Opus selbst (cpal) — vorher lief er zusätzlich über die WHEP-Verbindung der App, also doppelt, und der Schieber regelte nur eine Hälfte. `ManagedHqStream.nativeAudio` schaltet den App-Weg stumm, gesetzt von der `NativePlayerSession` (nicht von der Kachel: das Fenster überlebt deren Unmount) UND zusätzlich deklarativ von der Kachel (`ensure()` kann eine BESTEHENDE Sitzung liefern, während der Manager frisch ist).
- **Das Fenster geht nur bei 10-bit-Streams auf** (2026-07-26): `useNativePlayer` ist die Erlaubnis, die Bittiefe des Streams die Bedingung. Der Zuschauer erfährt sie aus der **WHEP-Antwort** (`ten_bit`) — sie reist im Token-Record mit, der auth-hook kopiert sie nach `stream:active`, media-svc gibt sie zurück (dieselbe Schiene wie `label`, s. `streaming/README.md`). Der Umweg ist nötig, weil die Kachel sich für einen Weg entscheiden muss, BEVOR dekodiert wird; 8 bit → `NativePlayerSession.skipped`, kein Fenster, still zurück ins `<video>`. **Bewusst NICHT über `stream:events`**: dort wird die `streams`-Liste weggelassen, solange alle auf Slot 0 sind (`poller.py::_is_multi`) — der Wert käme im Normalfall nie an.
- **In-App wäre 10 bit nicht möglich** (nicht „noch nicht"): jeder Weg, der Pixel ins Electron-Fenster bringt, läuft durch Chromiums 8-bit-Puffer. Fremde Surfaces einbetten kann Wayland prozessübergreifend nicht (Subsurfaces nur innerhalb EINES Clients), und ein Client darf seine Fensterposition nicht selbst setzen (Overlay-Trick fällt aus). Mit Tauri ginge es technisch (GTK-Fenster im eigenen Prozess → eigene Subsurface mit eigenem Format), scheitert aber an WebKitGTK-WebRTC = kein Voice, dem Grund für den Electron-Pivot.
- **Warum überhaupt** (gemessen 2026-07-26): Chromium legt seinen Wayland-Puffer **immer** als `ABGR8888` an — auch mit `--force-color-profile=scrgb-linear` und auch mit HDR, wo es PQ signalisiert und trotzdem 8 bit liefert. KWin bietet daneben `AB30`/`AB4H` an. Zusätzlich nutzt Chromium auf Linux/NVIDIA **kein NVDEC** (`nvidia-smi dmon`: `dec` durchgehend 0 %, auch mit VA-API-Flags). Beide Entscheidungen trifft der Player selbst.
- **AV1-Depacketizer ist gegen echte Daten geprüft** (2026-07-26): Rundlauf über den `Av1Payloader` des `rtp`-Crates, byte-exakt bei MTU 1200/300/100/20 inkl. Fragmentierung, und der rekonstruierte Strom dekodiert dieselbe Bildzahl wie das Original. **Falle für den nächsten, der dort testet:** `rtp` 0.17.2 hat einen Fehler in `codecs::av1::leb128` — `encode_leb128` packt die 7-Bit-Gruppen in 8-Bit-Slots, `put_leb128` liest mit `>>= 7` aus, also entsteht ab Wert 128 ein Müllbyte (`put_leb128(474)` → `[0x83,0xB4,0x03]` statt `[0xDA,0x03]`; selbst das crate-eigene `read_leb128` liest daraus 55811). Der Test korrigiert das vor dem Einspeisen. Wer das nicht weiß, diagnostiziert den Fehler des Generators als eigenen — mir ist das passiert, inklusive Commit d24109bd mit falscher "OFFENER FEHLER"-Meldung. Der Payloader wird produktiv nirgends benutzt.
  Nebenbefund aus derselben Analyse, und der Grund für das Symptom: die Schutzabfrage `rest.len() < n + len` in `push()` hat korrekt gegriffen (1200 < 3+55811) und die Einheit **sauber verworfen** statt Bildmüll zu bauen. Deshalb sah es nach Totalausfall aus, nicht nach Beschädigung — der Assembler weist kaputte Eingaben also nicht nur richtig ab, das Symptom war sogar diagnostisch.
  Bekannte Deckungslücke: der Payloader strippt das Size-Bit immer (`obu.header & !OBU_HAS_SIZE_BIT`), deshalb berührt der Rundlauf den Zweig "Größenfeld schon vorhanden" in `append_obu_with_size` nie. Nur per Unit-Test abgedeckt, nicht gegen echte Daten.
- **Restrisiko (Backlog, heute gedeckt)**: geht ein MITTLERES Fragment verloren, erkennt die Z/Y-Prüfung das allein nicht — das überlebende Fortsetzungspaket trägt selbst `Z=1`. In der echten Kette fängt `jitter.rs` das über die Sequenznummern ab und ruft vorher `on_gap()`. Ohne diese Meldung lieferte der Assembler stillen Bildmüll.
- **AV1-Depacketizer ist Eigenbau** (`src/depacket/av1.rs`): das `rtp`-Crate hat für AV1 nur einen *Payloader*. Der unangenehme Teil ist das Wiedereinsetzen der `obu_has_size_field`-Größenfelder, die RTP weglässt und FFmpeg erwartet. Acht Unit-Tests, **aber nie gegen einen echten Stream geprüft** — dort zuerst hinschauen, wenn AV1 schwarz bleibt.
- **Bewusst kein `SampleBuilder`**: der erledigt Umsortieren + Zusammensetzen in einem, versteckt dabei aber die Puffer-Entscheidung. Eigener Jitter-Puffer, weil die Fernsteuerungs-Messung 5–15 ms als ausreichend ergab und Chromium sich nicht dorthin zwingen lässt.
- **Kein libplacebo** (bräuchte FFI); Deband/Dither/Zoom stecken in `render/shader.wgsl`. **Kein GPL im Binary** — FFmpeg muss in Auslieferungen LGPL + dynamisch sein (System-FFmpeg auf Arch ist GPL-3.0, nur für Dev). Neue Abhängigkeiten gehören nach `THIRD-PARTY-NOTICES.md` **und** `web/src/lib/legal/drittanbieter.md`.
- **Ton + Mitschnitt seit 2026-07-26**: Opus-Decode via FFmpeg → cpal (`audio.rs`, eigener Thread, weil `cpal::Stream` nicht `Send` ist); Mitschnitt muxt den Bitstrom **ohne Neukodierung** (`recorder.rs`), plus 60-s-Ringpuffer für `clip`. **Container je Codec, gemessen:** H.264 → MPEG-TS (nimmt Annex B nativ; Matroska verlangt `avcC` + Längen-Präfixe und lehnt ab), AV1 → Matroska (MPEG-TS trägt AV1 nicht → `bin_data`; braucht den AV1CodecConfigurationRecord als `extradata`, gebaut aus dem Sequence-Header). Zeitstempel MÜSSEN in die Zeitbasis des Muxers umgerechnet werden (`rescale_ts`) — sonst 90 Bilder in 49 ms. Der Player setzt die Endung selbst und meldet den benutzten Pfad zurück. Ein Clip startet immer am letzten Keyframe davor. **Zielpfad bestimmt der Electron-Hauptprozess**, nie der Renderer (sonst Schreibzugriff an beliebige Stelle) — deshalb laufen `record`/`clip` über eigene IPC-Kanäle statt über `player:call`.
- **Der Ton bündelte das Bild — behoben 2026-07-26, und die Ursache lag NICHT im Player.** Symptom: sichtbares Ruckeln bei tadellosen Zahlen (Bildzahl, Bitrate, Paketverlust alle in Ordnung). Ursache: FLV ist EINE Zeitleiste, der Muxer im Sender gibt ein Bild erst frei, wenn Ton mit passendem Zeitstempel vorliegt — bei 20-ms-Opus-Paketen und 21-ms-PipeWire-Raster gingen die Bilder in 20-ms-Bündeln heraus. Behoben an der Quelle im Linux-Sidecar (5-ms-Opus + `node.latency=240/48000`), **nicht** am `max_interleave_delta`: daran zu drehen half nur einer Bildrate und ließ bei 280 fps die Schreibreihenfolge kippen (`write_interleaved: Invalid argument`, Stream tot). Messwerte und Begründung stehen im Sidecar-Repo an den Setzstellen.
- **Latenz ist vermessen, Ende zu Ende** (2026-07-26, Profile in `streaming/testbench/profiles/`, append-only): bei 2560x1440/60 fps/AV1/10 bit/4000 kbps **96 ms**, bei 144 fps **67 ms**. Aufteilung: Encoder **zwei Bilder Vorlauf** (33,3 bzw. 13,9 ms — größter Einzelposten, der Windows-Sidecar setzt für denselben Encoder `zerolatency=1`+`delay=0`, der Linux-Sidecar nicht), Aufnahme ein weiterer Bildabstand, Player von Ankunft bis Schirm nur 3,4 ms, und ein **Fixposten von 42,5 ms außerhalb unseres Encoders** (MediaMTX + Empfangsweg + Anzeigeverzug des Messfensters, noch nicht getrennt). Belegt über zwei Bildraten: die Differenz von 29,4 ms geht restlos in „2 Bilder Encoder + 1 Bild Aufnahme" auf, der Rest bleibt gleich groß. **Verfahren**: das Bild trägt die Uhrzeit selbst als Klotz-Muster (`latency-pattern.py` malt, `pulse-player/src/probe.rs` liest aus der Luma-Ebene zurück) — Zeitstempel im Bitstrom taugen nicht, weil FLV/RTMP/MediaMTX/WebRTC sie umschreiben.
- **Encoder-Vorlauf abgeschaltet** (2026-07-26, Linux-Sidecar `opts.rs`: `zerolatency=1` + `delay=0`, wie Windows es immer hatte; `PULSE_NVENC_LOW_DELAY=0` schaltet zurück): Encode-Latenz **33,4 → 2,9 ms**. Beim Zuschauer kommen davon aber nur **10 ms** an (96 → 86 ms Ende zu Ende) — etwa 20 ms werden dahinter wieder aufgezehrt, passend zu einer Station, die nach Zeitstempeln ausgibt statt nach Ankunft (Verdacht: MediaMTX, dieselbe Ecke wie der 42-ms-Fixposten). **Der nächste Latenz-Hebel liegt damit nicht mehr an unserem Encoder.** Offen: Einfluss auf die Bildqualität ist NICHT gemessen (dafür fehlt eine verlustfreie Referenzaufnahme im Sender; die Zeitmuster-Messung taugt nicht, weil ihr Bildinhalt fast schwarz ist).
- **Bildqualität ist messbar geworden** (2026-07-27): der Sender kann mit `PULSE_DUMP_RAW` den Encoder-**Eingang** verlustfrei mitschreiben (`raw_dump.rs` im Sidecar-Repo, 660 MB/s, Grenze 180 Bilder), der Player nimmt den empfangenen Strom auf, `compare-quality.py` ordnet beide über die Bildinhalte zu (nicht über Zeitstempel — zwei Uhren mit unbekanntem Versatz) und misst VMAF/PSNR/SSIM. Erstes Ergebnis: der abgeschaltete Encoder-Vorlauf kostet **0,6–1,4 dB PSNR und 0,010–0,016 SSIM** (zwei unabhängige Paare, gleiche Richtung; VMAF entscheidet nichts, ±1). Also 10 ms Latenz gegen rund 1 dB — Vorlauf bleibt ab, rückholbar mit `PULSE_NVENC_LOW_DELAY=0`. **Absolute Werte nicht überinterpretieren**: Testinhalt ist `testsrc2`, ein Rauschmuster, bei 0,018 bit je Bildpunkt.
- **Das Mittelstück der Latenz ist eine feste ZEIT, kein Puffer** (2026-07-27, Profil `latenz-2026-07-27-mittelstueck.json`): Kette jetzt vollständig direkt gemessen — Anzeige→Encoder **18,7 ms** (neu: `dump-latency.py` liest das Zeitmuster aus dem Rohmitschnitt und vergleicht mit der Wanduhr in der `.pts`-Liste), Encoder 2,9, Player 3,2, Ende zu Ende 90 → **Mittelstück 65 ms**. Vier Vermutungen geprüft und **widerlegt**: AVIO-`flush_packets` (kein Gewinn), ein datenmengen-abhängiger Puffer (bei 3,3-facher Bitrate nur 7,5 ms weniger — ein 32-KB-Puffer hätte 45 gebracht), Nagle (nur 3,6 ms, aber behalten: `tcp_nodelay=1`), die Mux-Warteschlange (kleiner = schlechter). Übrig bleiben MediaMTX und der pion-Empfangsweg; sie zu trennen braucht einen Paketmitschnitt auf der Schleife, der Keyframe-Bündel im Sender-Log gegen den UDP-Ausgang stellt. **Nicht ohne diese Trennung weiter raten.**
- **Das Mittelstück ist gefunden und halbiert** (2026-07-27 nachts, Profil `latenz-2026-07-27-mittelstueck-geloest.json`, Sidecar-Commit `eb0e7ad`): Es lag **weder in MediaMTX (unter 1 ms) noch im Empfangsweg des Players (0,04 ms)** — beide direkt gemessen und zusätzlich am Quelltext bestätigt. Es lag im **eigenen Sender**: `av_interleaved_write_frame` gibt ein Bild erst frei, wenn Ton mit mindestens so großem Zeitstempel vorliegt, der Rückstand des Tons ist also 1:1 Bild-Latenz. `max_interleave_delta` 100 → **10 ms**: Ende zu Ende **99,8 → 82,3 ms** (60 fps), 62,5 → 52,7 (144), 676 → 301 (280). Kein Schreibfehler, Ton lückenlos. Nicht kleiner, weil 2 ms bei 280 fps den Stream tötet (2026-07-26). Vier Dinge, die man wissen muss:
  - **Der Ton-Rückstand ist nicht konstant** — über zehn Läufe 12 bis 34 ms, je nachdem wie der PipeWire-Graph beim Start steht. Ein einzelner Vorher/Nachher-Vergleich beweist hier **nichts**; genau daran ist mein erster vermeintlicher Fix gestorben (Latenz-Anforderung am Null-Sink des Routers sah als Einzelmessung nach 33,5 → 21,4 aus, war über je fünf Läufe aber 22,5 gegen 21,3 — also Rauschen, Änderung zurückgenommen). Nebeneffekt des Deckels: die Latenz wird auch **gleichmäßiger** (82,5/82,6/82,7 gegen 99,2/98,2/102,0).
  - **Gleichförmige Ströme sind nicht korrelierbar.** Der erste Anlauf über Kreuzkorrelation der Byte-Verläufe lieferte Spitzen auf jedem Vielfachen von 40 ms — unbrauchbar. Was trägt: dem Sender fünfmal eine 120-ms-Pause aufzwingen (SIGSTOP/SIGCONT) und die Lücken zuordnen. Werkzeug: `streaming/testbench/split-latency.py`. **Muss beide IP-Familien lesen** — ffmpeg löst `localhost` zu `::1` auf, der ICE-Ausgang liegt auf `127.0.0.1`; wer nur IPv4 parst, sieht null Pakete auf der Eingangsseite.
  - **Der Bildschirm muss wach sein.** Schlafen die Monitore, nimmt der Sender Schwarz auf, das Zeitmuster ist weg und Ende zu Ende meldet `0.0 ms (60 ohne Muster)`. Bei Messreihen `systemd-inhibit --what=idle:sleep` setzen. Diagnose, die das aufdeckte: ein Einzelbild aus `rec-*.mkv` ansehen.
  - **Der Portal-Dialog kann ausbleiben statt zu erscheinen.** Der Berechtigungsspeicher (`flatpak permission-list screencast`) sammelt Einträge an, die auf ein **Fenster** zeigen; ist das Fenster weg, scheitert die Wiederherstellung stumm (Portal-Log: „Fenster-ID kann nicht gefunden werden") und der Sender endet sofort, ohne zu fragen. Alte Einträge löschen (`flatpak permission-remove screencast <id>`, **ohne** App-Kennung), Portal neu starten, und einen **Bildschirm** wählen, kein Fenster.
- **Neue Messschalter**: Player `PULSE_PLAYER_ARRIVAL_GAP_LOG_MS=<ms>` (meldet Ankunftslücken mit der **Wanduhr** — nur so mit tcpdump vergleichbar), Linux-Sidecar `PULSE_MUX_LATENCY_LOG=1` (Ton-Rückstand im Muxer; misst die **Ursache**, sieht `max_interleave_delta` NICHT — wer den Deckel bewerten will, misst Ende zu Ende).
- **Prüfstand `streaming/testbench/`** — fährt einen echten Stream ohne App, ohne Klick (Token selbst in Redis, Sender starten, Player über stdio öffnen, Messwerte einsammeln). Zwei Sender: ffmpeg als bekannt gleichmäßige Referenz (trennt „unser Sender" von „dahinter") und der echte Sidecar. Die aussagekräftige Zahl ist **nicht** die Bildrate, sondern `zu spaet` (Ausgabe-Abstände über dem doppelten Soll) und `Ankunft max`. Portal-Dialog nur beim ersten Lauf (`PULSE_PORTAL_REUSE=1`).
- Offen: echte A/V-Synchronisierung (heute nur Puffer-Näherung), Standbild-Export, zero-copy, Windows/macOS ungetestet. Ab ~280 fps bündelt es wieder — dort ist aber die AUFNAHME am Anschlag (der Compositor meldet `max_framerate 279` und der Sender verdoppelt 16-34 Bilder je Sekunde), das ist ein eigener Faden.
