...
- **Code-Simplifier nach jeder Code-Änderung (erzwungen):** `code-simplifier`-Agent über die geänderten Dateien → relevante **Tests/Checks erneut grün ziehen** (das ist die Kontrolle, ob die Vereinfachung etwas gebrochen hat — kein zweiter Review-Agent nötig) → `bash .claude/hooks/simplify-stamp.sh` → committen. Zwei Hooks erzwingen das, beim `git commit` und am Turn-Ende; ohne Stempel blockieren sie. **Mechanik, Ausnahmeliste und was zu tun ist, wenn der Gate nicht greift: `.claude/hooks/README.md`.**
