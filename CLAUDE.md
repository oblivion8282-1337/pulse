...
- **Rust-Linux-Sidecar liegt im Repo** (`streaming/linux-hq-sidecar/`); Flatpak baut ihn per `type: dir` → Änderung löst Flatpak-Build aus. Dev braucht `$PULSE_LINUX_HQ_SIDECAR` (setzt `dev-up.fish` wenn gebaut). Bei `Cargo.lock`-Änderung `packaging/linux-hq-sidecar-cargo-sources.json` neu generieren (Flatpak baut Cargo offline). Messbegründungen zu Encoder-/Puffer-Werten: `docs/2026-07-30-linux-hq-sidecar-messbegruendungen.md`. **10 bit auf NVIDIA:** was es je Stufe kostet, welche Hebel offen sind (`highbitdepth=1` statt P010-Shader, `cuCtxSynchronize` je Bild, Puffer-Rückgabe vor dem Import) und warum man 10 bit unter KDE sieht, unter niri hier aber nicht: `docs/2026-09-08-zehnbit-linux-nvidia-analyse.md`.
...
- **Instanz-Rolle** (Env, chat-gateway *und* auth-svc): `PULSE_INSTANCE_MODE` = `cloud`|`self-host` (**Default `self-host`!**, prod-Cloud `.env` setzt `cloud`); `PULSE_INSTANCE_ID` (0 = Cloud; ≥100 = von Cloud vergeben); `PULSE_INSTANCE_OWNER_ID` (chat-gateway) = Cloud-User-ID des Owners → beim Anmelden wird `ticket.sub == owner_id` **automatisch Admin**; `ALLOW_LOCAL_ACCOUNTS` (auth-svc, default false) = lokaler Passwort-Escape.
- **Registrierung** (`auth_settings.registration_mode`: open|invite_only|closed): Self-Host (`mode != cloud`) **blockt `POST /register`** by default → Identität per Cloud-Ticket. `invite_only` verlangt Code (`registration_invites`, Migration 0022, atomar guarded-UPDATE; Deep-Link `…/register?invite=CODE`).
- **Anmeldung am Self-Host = Cloud-Ticket** (seit 2026-08-28, ersetzt das Gerätezertifikat). Der Browser holt bei der Cloud (`POST /me/server-ticket`, `dcc_auth/server_ticket.py`) ein auf **genau eine Instanz** ausgestelltes JWT mit 60 s Laufzeit und legt es dem Server vor (`POST /session`, `routes/session_ticket.py`). Der prüft die Signatur gegen die zwischengespeicherten Cloud-JWKS und gibt eine **eigene Sitzung, 1 h** (`SITZUNGSDAUER_S`). **Im Browser liegt nichts Langlebiges mehr** — kein Schlüsselpaar, kein Zertifikat, keine IndexedDB-Identität. Drei Eigenschaften ersetzen zusammen die frühere Nonce-Signatur, keine allein: Frist, `aud`-Bindung, Einmal-Einlösung über `jti` (`ticket_pruefung.py`). **Der Klient wählt den Weg nicht mehr** — es gibt nur einen; `/.well-known/pulse-server-info` nennt die Instanz-Kennung und die Fähigkeiten (`faehigkeiten.py`, eine Quelle für Server-Info UND `hello`).
- **Es gibt keinen Zertifikats-Widerruf mehr.** `routes_credentials.py`, `routes_crl.py`, `credential_revocation.py` und die Tabellen `issued_credentials`/`revoked_credentials` sind am 2026-08-28 entfallen (Migration 0051). Ein Ticket lebt 60 s — es gibt nichts nachträglich zurückzuziehen. **Was eine Sperre heute bewirkt:** Die Cloud stellt einem `disabled`-Konto kein Ticket mehr aus (`_require_user` → 401), bestehende Self-Host-Sitzungen laufen binnen **einer Stunde** ab. Der Nachlauf ist damit nicht null, aber statt der früheren 365 Tage. Sofort wirken könnte eine Nutzersperre nur, wenn die Cloud sie an jede Instanz meldete — das tut sie heute nur für gesperrte **Instanzen** (`suspend_poller`).
  - **Historie, weil sie die Bauform erklärt:** Das `device_label` (`<Browser> · <OS>` ohne Rechnernamen) taugte nie als Geräte-Identität — Chrome, Edge, ein zweites Profil und ein Inkognitofenster trugen alle `Chrome · Windows`. Eine Neuausstellung zog jeden aktiven Pass mit gleichem Label zurück; zwei Browser meldeten sich dadurch endlos gegenseitig ab, und **eine Neuausstellung half nie, weil sie den Zustand gerade erzeugte**. Der Vorfall am 2026-08-28 war der Anlass für den Ticket-Umbau. Die Lehre trägt weiter: Ein Etikett, das ein Browser aus Privacy-Gründen nicht eindeutig machen kann, ist keine Identität.
...
- **well-known-Endpoints** (auth-svc, Root): `/.well-known/{jwks.json, pulse-version-policy.json, pulse-suspended-instances}` — Self-Hosts pollen: `jwks_poller.py` (jwks, 30 s; hiess bis 2026-08-28 `crl_poller` und holte zusätzlich die entfallene Zertifikats-Sperrliste), `cloud_policy_poller.py` (version-policy, 6 h), `suspend_poller.py` (suspended-instances, 60 s). Sperre verweigert die Anmeldung (403 `instance_suspended`/`instance_deleted`) + trennt WS (**4070**, seit 2026-08-17 eigener Schliesscode — vorher teilte sich 4003 mit dem E-Mail-Riegel, jetzt 4071, und der Klient las beides als „CORS blockiert"; Codetabelle `routes/ws.py` ↔ `web/src/lib/api/constants.ts` **synchron halten**). **Fail-open ist Absicht** (Cloud/Redis nicht erreichbar = „nicht gesperrt", sonst legt ein Cloud-Ausfall alle Self-Hosts lahm); Daten unangetastet, Container nicht gestoppt (Sperre umkehrbar, sonst Neustartschleife). **`web-nginx.conf` muss sie per Regex-Location an auth-svc routen** (sonst SPA-Fallback → Poller scheitern still mit JSONDecodeError). `acme-challenge` ausgespart.
  - **Der ETag der Sperrliste MUSS an die eigene Instanz-ID gebunden sein** (`suspend_poller.py::_etag_marke`, seit 2026-08-27). Der ETag beschreibt die **Liste**, der in Redis abgelegte Zustand ist aber (Liste × **eigene ID**) — ein Cache-Schlüssel, dem die halbe Eingabe fehlt. Wer seinen Server neu aufsetzt, wechselt die ID, ohne dass sich die Liste ändert: jeder Abruf bekam 304, der Poller kehrte vor `_state_for` zurück, und der `deleted`-Zustand der **Vorgänger**-Instanz blieb stehen — bis irgendwo auf der Welt eine beliebige andere Instanz gesperrt wird. Hat einen echten Server 24 h lahmgelegt (Voice und Bildschirmübertragung inklusive: ohne WS und ohne Anmeldung entsteht kein Token mehr), und in der Cloud stand er die ganze Zeit auf `active`. Ein Wert ohne `|` gilt seither als ungültig — der Bestand heilt sich beim ersten Abruf nach dem Update selbst.
  - **Cloud-seitige Notbremse, wenn ein Bestandsserver so feststeckt und man keinen Zugriff auf ihn hat:** den jüngsten `suspended_at` in `auth.suspended_instances` um 1 ms schubsen und `auth:suspended_instances:{etag,body}` in der Cloud-Redis löschen (Cache-TTL ist sonst 1 h). Das kippt den ETag für **alle** Poller, ohne die Liste inhaltlich anzufassen; der betroffene Server ist binnen 60 s frei. Am 2026-08-27 so gemacht und nachgemessen (4070 → 4001).
...
- **Erreichbarkeits-Diagnose (seit 2026-08-25)** — ein Self-Host muss acht Glieder hintereinander bestehen (DNS, TCP/443, Zertifikat, fremder Proxy, CORS, **WebSocket-Upgrade**, UDP-Medienports); bis dahin fasste jede Stelle alles davon zu einem „nicht erreichbar" zusammen. **Zwei Glieder stellen keine Erreichbarkeitsfrage: „erkennt dieser Server dich als Betreiber?" (seit 2026-08-27) und „spricht er den heutigen Anmeldeweg?" (neuntes Glied, seit 2026-08-28 — ein Server ohne die Fähigkeit `server-ticket` nimmt niemanden mehr auf, `selfhost_probe_anmeldeweg.py`)** (`selfhost_probe_betreiber.py` ↔ chat-gateway `routes/owner_check.py`). **Voll-Doku: `docs/selfhost-erreichbarkeit.md`.** Was man ausserhalb wissen muss:
...
- **Eine Behauptung wird nie an nur EINER Stelle korrigiert — vorher greppen.** Wer einen Wert/Pfad/Optionsnamen/Version/eine Verhaltensaussage ändert, sucht erst alle Fundstellen (`grep -rn "<alter Wert>"`) und zieht sie mit. Anfällig: IPs/Hostnamen, Versionsnummern (Electron/MediaMTX/FFmpeg), Env-Variablennamen, Encoder-/Optionsnamen, Ops-Listen, Lizenzbezeichnungen — alles, was gleichzeitig in `CLAUDE.md`, einer `README.md` und einem Code-Kommentar steht.
  - **Die Regel gilt fürs LÖSCHEN genauso**, und dort wird sie verlässlich vergessen: der Pfad einer Datei ist eine Behauptung wie jede andere. Am 2026-08-27 kostete das zwei rote Bauten — `streaming/pyproject.toml` fiel mit dem GSR-Ausbau weg, und `Dockerfile.service` (die sechs Cloud-Dienste) wie `infra/self-host/Dockerfile` kopierten sie weiter. Vor einem `git rm` also `grep -rn "<pfad>"`, nicht danach.
  - **Zwei blinde Flecken, die derselbe Lauf offenlegte.** (1) **Bau-Rezepte**: `Dockerfile*`, Compose-Dateien und `.github/workflows/**` nennen Pfade, die in keinem Import-Graphen stehen — eine Abhängigkeitskarte, die aus dem Quellcode entsteht, kennt sie nicht. Wer sich auf eine solche Karte verlässt (auch auf eine von einem Leselauf erstellte), greppt trotzdem selbst. (2) **Lizenztexte**: `LICENSE*`, Verzeichnis-`LICENSE`s, `THIRD-PARTY-NOTICES.md` und `web/src/lib/legal/drittanbieter.md` führten nach dem Ausbau weiter eine GPL-Komponente, die gar nicht mehr ausgeliefert wurde — die letzte davon ist die Seite, die NUTZER sehen. Sie brechen nichts und fallen deshalb in keinem Test auf.
