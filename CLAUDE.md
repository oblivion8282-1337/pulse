...
- **AV1-WIEDERGABE IST KAPUTT (offen, Stand 2026-07-26)**: ein Rundlauf-Test über den `Av1Payloader` des `rtp`-Crates zeigt, dass echte Ströme nicht korrekt zusammengesetzt werden — erste Zugriffseinheit schon unvollständig, bei jeder MTU. Die beiden Tests sind `#[ignore]`, damit der Lauf grün bleibt; nachvollziehbar mit `cargo test depacket -- --ignored` und gesetztem `PULSE_PLAYER_AV1_FIXTURE`. Da AV1 der Standard-Codec ist, taugt der Player bis dahin nur für H.264.
