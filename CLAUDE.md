...
- **Nonce gegen Republish-ICE-Race**: jeder Token-Issue → frische 8-Hex-Nonce → Pfad `channel-<cid>-<uid>-<nonce>` (gleicher Pfad < Sek. später = tote Session; ursprünglich MediaMTX-1.17.1-Bug #913, upstream in 1.18+ via pion/ice v4.2.5 gefixt — Nonce bleibt als Defense-in-Depth). `stream:active:*` hält den Live-Pfad **ohne** Nonce für WHEP-Lookup.
