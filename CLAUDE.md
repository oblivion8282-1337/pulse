...
- **Die Aufnahme-GPU ist wählbar** (seit 2026-08-17, `system::gpu_wahl` + Patch `0002` an windows-capture). **Der Hebel sitzt an der AUFNAHME, nicht am Encoder**: `windows-capture` baute sein Gerät mit `D3D11CreateDevice(None, …)`, Windows gab die Karte des primären Bildschirms, und `pipeline_hw` leitet Hersteller *und* Encode-Weg aus genau diesem Gerät ab — ein Encoder-seitiger Schalter (`select_adapter`) lief deshalb ins Leere. Jetzt reicht `StartParams::gpu` die Wahl bis in `CaptureConfig::gpu` durch (**alle drei Pipelines**). Automatik = meiste Menge **eigener** Videospeicher (`vram_mb`) unter den Herstellern mit Zero-Copy-Weg (`vendor_traegt_zero_copy`) — der Speicher entscheidet mit, weil die DXGI-Reihenfolge eine im Prozessor eingebaute Radeon-Grafik nicht von einer eingesteckten AMD-Karte trennt. Renderer wählt über `overrides.gpu` = opake `id` aus `gpu_info.adapters[]` (`"10DE:1E84"`); unbekannte `id` → Automatik, kein Abbruch. `health`/`gpu_info` melden oben die **automatisch gewählte** Karte, nicht mehr `adapters[0]`.
- **Intel kann über WHIP gar nicht senden** (also praktisch nie, Pulse schickt H.264 immer über WHIP): `encode_path` gibt bei angemeldetem Sendeweg für JEDEN Hersteller `D3d11ZeroCopy` (`codec.rs`, Senke-Kurzschluss), aber `h264_qsv` nimmt keinen D3D11VA-Rahmenspeicher → `avcodec_open2` = `Invalid argument`. Der CPU-Weg wäre keine Rettung: `encoder.rs::create` öffnet **immer** den ffmpeg-Muxer und kennt `encode::senke` nicht. `run_pipeline` bricht die Kombination seit 2026-08-17 mit lesbarer Meldung ab — **die Heilung fehlt noch** (CPU-Weg müsste den Sendeweg bedienen können).
...
- **`git tidy`** nach gemergtem PR laufen lassen (löscht lokal die server-seitig bereits gelöschten Head-Branches). Einmalig pro Maschine setzen: `git config --global fetch.prune true` + den Alias.
  ...
  Zwei nicht-offensichtliche Stücke (beim Kürzen nicht wegfallen): **`LC_ALL=C`** Pflicht (sonst schreibt git auf deutscher Maschine `: entfernt]` statt `: gone]` → Muster greift nie, räumt still gar nichts). **`git cherry`** statt `--merged`/`-d`: bei Rebase-Merges haben gleiche Änderungen andere Prüfsummen, `--merged`/`-d` halten den Branch fälschlich für ungemergt — mit `-D` wäre er wortlos weg.
