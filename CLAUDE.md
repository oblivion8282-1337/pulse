...
- **Windows-Release braucht IMMER einen Version-Bump**: über den Installer ausgelieferte Änderungen (`streaming/win-hq-sidecar/**`, `desktop/electron/**`, `desktop/package.json`-Deps) erreichen Bestandsclients nur, wenn `version` gebumpt wird — electron-updater ignoriert gleiche Version stillschweigend. Linux (Flatpak) hat das Problem nicht.
