...
- **Windows-Distribution = NSIS-Installer + electron-updater-Auto-Update** (`dist:win`): pollt `https://howispulse.com/updates/win/latest.yml`, verifiziert per SHA512, unsigniert (SmartScreen nur beim Erst-Download; `signAndEditExecutable` bettet nur Icon/Version per rcedit ein, **kein** echtes Code-Signing). **Zwei Update-Schichten**: (1) **Start-Splash** — `bootWithUpdateCheck()` (`main.ts`) öffnet VOR dem Hauptfenster ein frameless Splash (`build-resources/update-splash.html`, extraResource → `process.resourcesPath`), prüft, lädt + **installiert vollautomatisch ohne User-Wahl** (`quitAndInstall(true,true)` 1 s nach `ready`); bei `updated:true` startet die Haupt-App gar nicht (sonst ~1 s Fenster-Blitz vor dem Installer-Quit). Bei `no-update`/`error` schließt der Splash nach 300 ms/1 s → normale Haupt-App. (2) **In-App** — `wireInAppUpdater` + `startPeriodicUpdateChecks` (Default 60 Min, `PULSE_UPDATE_INTERVAL_MS`) für Updates, die während der Session entdeckt werden; abbrechbarer sonner-Banner (`window.pulse.updates.*`, `autoRestart:false`). `autoUpdater.allowDowngrade = false` in `updater.ts` (**beide Pfade**) — eine CI-Misconfig, die versehentlich eine alte Version als „latest" pusht, rollt Clients NICHT zurück. **Kein Downgrade-Notfallnetz mehr**: eine kaputte Live-Version lässt sich NICHT mehr durch `latest.yml`-Rückstellung zentral zurückziehen (früher `=true`) — dafür müsste der Schalter geflippt + neu ausgeliefert werden. Logik `electron/updater.ts` (gated `app.isPackaged && win32`; Test-Modus `PULSE_TEST_UPDATE=1` mit `PULSE_TEST_*_MS`-Phasen, `updater.log` unter `<userData>/`). CI `.github/workflows/win-build.yml` scpt den Feed (kein `--delete` → Delta); **win-build + flatpak triggern nur auf `main` (deployen direkt auf den Prod-Feed) → nicht als PR-Check verifizierbar**, erst nach Merge sichtbar. Voll-Doku: `docs/plans/2026-05-31-windows-auto-update.md`. **Globaler Hold-to-Talk-Shortcut fehlt** (Electrons `globalShortcut` kann nur Press, kein Hold); In-Window-PTT läuft über das zentrale `lib/shortcuts/`-System + `desktop/electron/shortcuts.ts` (drucktasten-basierte globale Shortcuts für andere Aktionen sind dort wiring-fähig). Native Notifications-IPC IST gebaut: `desktop/electron/notify.ts::wireNotify()` (in `main.ts` gebootet), Renderer-Trigger via `window.pulse.notify.*`.
...
**Self-Host-Approval-Flow = Single-Bootstrap pro Antrag**: User stellt Antrag → Super-Admin genehmigt (`POST /admin/instance-applications/{id}/approve`) → `registered_instances`-Eintrag wird active → User mintet **genau einmal** einen Bootstrap-Token (`POST /me/instances/{id}/bootstrap-token`) und löst ihn auf seinem Server ein (`POST /selfhost/bootstrap`). Nach erfolgreichem Redeem (`consumed_at IS NOT NULL`) sind weitere Mints geblockt — User muss für jeden weiteren Server einen neuen Antrag stellen. Der `users.self_host_enabled`-Schalter greift hier nicht (Owner-Check `registered_by == user.id` reicht); er bleibt nur für den parallelen Env-File-Download-Endpoint (`generate_env_file`) aktiv, weil dort jederzeit ein neues `client_secret` rotiert werden können muss. Container-Crash-Recovery nach erfolgreichem Setup ist trotzdem möglich — der Container nutzt seine persistierten `client_id`/`client_secret` direkt, ohne erneuten Bootstrap-Redeem.
...
**Account-basierte Server-Liste** (ersetzt den E2E-Server-Vault ab 2026-06-28): Die Self-Host-Server-Liste lebt jetzt in `auth.user_instance_memberships` (Cloud-DB, Migration 0037) statt clientseitig verschlüsselt in einem per-User-Vault. Beim Bootstrap-Token-Redeem (`routes_selfhost_bootstrap.py`) wird die Owner-Membership automatisch eingetragen; `GET /me/instances` liest sie als JOIN. Inhalts-Privacy bleibt unverändert (Cert-Modell: Self-Hosts sind isolierte DB-Welten); nur die sekundäre Tracking-Dimension der persönlichen Server-Liste wird preisgegeben — das war schon immer eine schwache Privacy-Garantie (Audit-Logs + Phase-4-6-Heartbeats leaken dieselbe Info). Der frühere Zero-Knowledge-Vault (`encrypted_server_vaults`, Migration 0026; Code in `lib/identity/vault-crypto.ts` + `server-vault.svelte.ts`) ist komplett entfernt. Erweiterung um eingeladene Nicht-Owner-User ist für Phase 4-6 vorgesehen — das `role`-Feld in der Membership-Tabelle ist dafür vorbereitet.
...
- **Branch-Aufräumen (gegen liegengebliebene Branches):** GitHub-Repo hat `delete_branch_on_merge=true` → der Head-Branch wird beim PR-Merge **server-seitig automatisch** gelöscht (gilt für alle Rechner). **Per-Maschine** noch einmalig setzen (Claude soll das beim ersten Mal auf einer neuen Maschine tun, falls nicht vorhanden): `git config --global fetch.prune true` + Alias `git tidy` (`!git fetch --prune && git branch -vv | awk '/: gone]/ {print ($1=="*")?$2:$1}' | xargs -r git branch -D`) → entfernt lokale Branches, deren Upstream weg ist. Nach gemergtem PR `git tidy`.
