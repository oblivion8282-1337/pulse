...
- **OFFEN auf `fix/hq-60fps-glaettung`: Zeitbasis nachziehen.** Der Windows-Sidecar stempelt Bilder seit 2026-08-14 in **1/90000** statt in Bildplätzen (`1/fps`) — nur so überlebt die echte Ungleichmäßigkeit der Abtastung (auf 143 Hz bei 60 fps: 13,9/13,9/20,8 ms statt dreimal 16,7). Der Linux-Sidecar steht noch auf `1/fps` und braucht dieselbe Änderung: Encoder-Zeitbasis, pts-Ableitung **und Duplikat-Zähler** in `stream_controller.rs`, dazu `whip/av1.rs::zeitstempel`. Vorlage `streaming/win-hq-sidecar/src/zeitbasis.rs`; die zwei teuren Fallen (Duplikate am Zähler, Lücken-Schwelle) stehen in `docs/2026-08-14-hq-60fps-glaettung-messanleitung.md`, Abschnitt „Feinere Zeitbasis". **Diese Zeile fällt weg, sobald es gemacht ist.**
...
- **Gotchas**: Secrets server-seitig in `.env` + `secrets/jwt_*.pem` (**PEM `chmod 0644`**, uid 10001). Avatar-Volume Fresh-Deploy `chown 10001:10001` (sonst Upload-500). UFW `7880`/`9997` nur vom Docker-Bridge (`ufw allow from 10.0.0.0/8`) — sonst blockt `INPUT DROP`. MediaMTX = **1.19.1-pulse3**-Fork (`infra/mediamtx-fork/`, TempDelim-Patch für AMD-VAAPI AV1, Image `ghcr.io/oblivion8282-1337/pulse-mediamtx:1.19.1-pulse3`); `:9997`-API-Schutz hängt **nur an der UFW** (`apiAllowAddresses` in 1.19 entfernt). Dev-compose läuft ebenfalls 1.19.1-pulse3. Migrate-Container laufen automatisch beim Deploy mit.
