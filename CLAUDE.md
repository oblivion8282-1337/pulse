...
- **OFFEN auf `fix/hq-60fps-glaettung`: Lücken-Schwelle korrigieren.** `linux-hq-sidecar/src/zeitbasis.rs` hat `lueckenschwelle` als wortgleiche Kopie übernommen, inklusive der **falschen** `* 3 / 2`. Anderthalb Bildabstände waren aus dem 60-fps-Fall allein hergeleitet; gemessen reicht die echte Abtast-Schwankung bei 120 fps bis 1,67 und bei 144 fps bis 2,00 Bildabstände (Tabelle am Windows-Wert). Zu ändern: `* 3 / 2` → `* 2` plus der Test `luecke_trennt_schwankung_von_ausfall`. **Diese Zeile fällt weg, sobald es gemacht ist.**
