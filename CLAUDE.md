...
- **Reconcile-Loop** (`voice-signaling/reconcile.py`, 2026-06-03): der Webhook-Pfad allein driftet von der Realität weg — (1) Webhooks gehen bei Deploy/Restart verloren (LiveKit re-feuert kein `participant_joined` für schon-Verbundene), (2) **NX-TTL-Trap**: `_apply_join` setzt die 6h-TTL per `NX` nur beim ersten Join, nie erneuert → in dauerbesetzten Channels läuft das Set ab, obwohl alle drin sind, und heilt erst beim Komplett-Leeren. Symptom: Members-Liste leer / „ausgeloggt" trotz aktivem Voice. Fix: ein Background-Task pollt LiveKit (`list_rooms`/`list_participants`) alle `voice_reconcile_interval_seconds` (default 30; einmal beim Start → deckt die Deploy-Lücke ab) und **überschreibt** die 3 Sets (room/streaming/camera) gegen die echte Teilnehmerliste via `_set_exact` (DEL+SADD+EXPIRE atomar, **non-NX** TTL), published `voice:events`, räumt Ghost-Channels ab. Reconcile ist jetzt die Autorität, die TTL nur noch Backstop. Reuse der webhook-Helper (Key-Namen + Screen-Share/Kamera-Klassifikation) zwingt beide Pfade konsistent. `voice_reconcile_enabled=false` → Webhook-only-Fallback.
- **`LIVEKIT_API_URL` (interner LiveKit-API-Pfad):** Server-seitige LiveKit-Calls (Reconcile + Admin-Mute/Disconnect) gehen über den `app.state.livekit_api`-Client. Default war die **öffentliche** `LIVEKIT_URL` (`wss://howispulse.com/livekit`) → Caddy→nginx→LiveKit. Problem: genau **während eines Deploys** ist die Web-Schicht kurz weg → der Startup-Reconcile (der ja den Deploy abdecken soll) crasht selbst mit 502 (`ContentTypeError`/twirp) und heilt erst Minuten später. Fix: optionales `LIVEKIT_API_URL=http://host.docker.internal:7880` (prod `.env`) → der API-Client (`app.py::_livekit_api_host`, prefer-internal-sonst-public) spricht LiveKit **direkt** am Host an, unabhängig vom Web-Layer. Braucht `extra_hosts: host.docker.internal:host-gateway` auf voice-signaling (wie media-svc→MediaMTX; UFW erlaubt Bridge→Host:7880). **Unset → Fallback auf `LIVEKIT_URL`** (dev/Tests = schon localhost). Browser-Clients kriegen weiterhin `LIVEKIT_URL` als ws_url.
...
**Watch-Party Host-sticky** (2026-06-02, geändert von Auto-Handoff): Der Host **behält** die Party, bis er sie *explizit* abgibt (`watch_handoff`). **Kein Auto-Handoff mehr** bei Host-Wegfall. Stattdessen: Channel-Wechsel / Kachel-Unmount (`watch_leave`, host) **beendet sofort** (`end_if_host`); ein echter WS-Disconnect (`cleanup_on_disconnect`, host) startet eine **30 s Schonfrist** gegen Blips (`end_or_grace_if_host` → `_WatchRegistryMixin.schedule_host_end`, in-process `asyncio.Task` in `_watch_end_timers`) — kommt der Host via `watch_join` zurück, bricht `cancel_host_end` den Timer ab, sonst löscht der Timer den State. `WATCH_HOST_GRACE_S` (env, default 30; E2E=1). Zuschauer-Abgang ändert nichts. `promote_or_end` (`routes/watch_handoff.py`, unter `manager._lock`, Re-Check `host_user_id`, Position extrapoliert, `is_playing` erhalten) existiert weiter, wird aber **nur noch** vom expliziten `handle_handoff` genutzt (Target- bzw. next-oldest-Pfad). Watcher-Menge = **in-process** im `ConnectionManager` (`watch_registry._WatchRegistryMixin`, user-granular mit Socket-Refcount → Multi-Tab-korrekt, kein Redis: einziger Schreiber ist das Gateway; Cross-Pod bewusst nicht). **UX:** Das PARTY-Badge zum Öffnen hängt an der Host-Voice-Präsenz — bei Disconnect ist es während der Schonfrist weg; wer schon zuschaut, schaut nahtlos weiter, neue Zuschauer haben in dem Fenster keinen Einstieg (bewusst akzeptiert). Ops: `watch_join`/`watch_leave` (Tile-Mount/-Unmount), `watch_handoff{target_user_id?}` (host-only, 4015 non-host / 4018 target-not-watching). `watch_watchers`-Broadcast (direkt per `_fan_out`, view-channel-gefiltert) speist Client-Store `watchWatchers` + den Handoff-Picker. Client-Sync-Orchestrierung liegt jetzt im `web/src/lib/watch/partyController.svelte.ts` (aus `WatchPartyTile.svelte` extrahiert, Größen-Cap). **WS-Tests lokal brauchen `PULSE_INSTANCE_MODE=cloud`** (sonst self-host-Guard `PULSE_INSTANCE_ID`-Crash im Lifespan).
