...
- **Windows-Distribution = NSIS-Installer + electron-updater-Auto-Update** (`dist:win`, pollt `.../updates/win/latest.yml`, SHA512, unsigniert): Start-Splash (`bootWithUpdateCheck()` in `main.ts`) installiert **vollautomatisch ohne User-Wahl** (`quitAndInstall(true,true)`) VOR dem Hauptfenster; zusätzlich In-App-Banner für Session-Updates (Default 60 Min, `PULSE_UPDATE_INTERVAL_MS`). **`allowDowngrade=false`** (beide Pfade, `electron/updater.ts`) → eine kaputte Live-Version lässt sich NICHT mehr per `latest.yml`-Rückstellung zurückziehen (kein Downgrade-Notfallnetz). Voll-Doku: `docs/plans/2026-05-31-windows-auto-update.md`.
...
- **Pfad-Filter-Konvention:** Neue/geänderte Build-Workflows bekommen `on.push.paths`, damit sie nur bei relevanten Dateien laufen. Stand: `win-build`/`mac-build`/`flatpak`/`allinone` haben Filter; **`ci.yml` (Tests + 7 Cloud-Images) bewusst NICHT** (Tests = Gate, müssen breit laufen; Cloud-Deploy hängt dran).
