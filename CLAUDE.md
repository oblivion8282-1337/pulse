...
- **Server** (`services/`, `shared/`, `infra/`) = **Pulse Server License 1.0** (eigener Text, 2026-07-29) — Quelle einsehbar, 32 Tage Evaluierung, danach kommerzielle Lizenz nötig. Das ist der Hebel fürs bezahlte Self-Hosting (Freischaltung + monatliche Servicegebühr).
- **Client** (`web/`, `desktop/`, `mobile/`, `streaming/`, `plugins/`, `packaging/`, `Logo/`, `scripts/`) = **Pulse Client License 1.0** (eigener Text, 2026-07-29) — **Nutzung frei** (Cloud-Nutzer brauchen keine Lizenz, sonst träfe die eigene Lizenz das eigene Cloud-Geschäft), **Quelle einsehbar**, aber **Ändern, abgeleitete Werke, Weitergabe und Wiederverwendung in anderen Projekten sind untersagt**.
- **Warum eigene Texte statt PolyForm** (Entscheidung 2026-07-29): PolyForm Perimeter erteilt ausdrücklich eine „Changes and New Works License" und eine „Distribution License" — es erlaubt also genau das, was hier verboten sein soll, und verbietet nur Konkurrenz. Free Trial erlaubt Änderungen während der 32 Tage. Eine geänderte PolyForm-Fassung darf nicht mehr so heißen, deshalb eigene Texte. Sie sind absichtlich schlicht: „alle Rechte vorbehalten" ist der gesetzliche Standard, gewährt werden nur Nutzung und Einsicht. **Anwaltlich nicht geprüft** — bei echtem Umsatz nachholen.
...
Genaue Versionen in `uv.lock` / `pnpm-lock.yaml` / `package.json`. Runtimes: **Python** 3.13 (`>=3.13,<3.15`) · **uv** · **Node** ≥20 (CI: `ci.yml`+`flatpak.yml` auf **25**, die App-Builds `win`/`mac`/`android` auf **22** — beim Anpassen beide Gruppen prüfen) · **pnpm** 10. Ruff `line-length=100`, `target-version=py313`, `ignore=["E501"]`.
...
**Snowflake-IDs als Strings über die API-Grenze** (REST, WS, Responses) — JS `Number` kann 64-bit nicht exakt. Backend-Schemas: `SnowflakeId`-`BeforeValidator` (int *oder* string); Frontend sendet immer string. Format `[42-bit ms ab 2026-01-01][10-bit worker][12-bit seq]`, Worker auth=1/chat=2. **voice-signaling vergibt keine IDs** (kein `worker_id`, kein `next_id` — es konsumiert nur fremde Snowflakes); die früher hier genannte „voice=3" gibt es im Code nicht.
...
- **Push = zwei Wege, und welcher genommen wird, entscheidet der Client** (`web/src/lib/stream/settings.svelte.ts::pushProtokoll` → `protocol` im Stream-Token-Request; media-svc erzwingt, der Wunsch hebt nur **nach oben** Richtung WHIP). **Hier stand bis 2026-08-07 „RTMPS-only" — das ist seit dem 2026-08-02 falsch.**
  - **WHIP** (`https://<host>/whep/<pfad>/whip?token=…`) bei Intra-Refresh **und seit 2026-08-07 bei JEDEM H.264-Stream**, auch mit abgewähltem Intra-Refresh. Grund: nur WHIP hat den RTCP-Rückkanal, und `h264_amf` fährt die Auffrischung wegen `usage=ultralowlatency` **ungefragt** mit — ein H.264-Strom hat also kaum Vollbilder, egal was das Kästchen sagt. Über RTMPS sieht ein beitretender Zuschauer dann **gar nichts**: am 2026-08-07 in der Produktion 0 dekodierte Bilder (25 unbeantwortete Anforderungen, 20 Neuaufbauten) gegen 2681 über WHIP, derselbe Kanal. FlexFEC-Parität gibt es ebenfalls nur über WHIP.
  - **RTMPS** (`rtmps://<host>:1936`, self-signed Cert, UFW `1936/tcp`; `rtmpEncryption: strict` — plain :1935 ist entfernt) nur noch für AV1 ohne Intra-Refresh. Kein Rückkanal, keine Parität.
...
- **Der Rust-Linux-Sidecar liegt seit 2026-07-29 IM Repo** (`streaming/linux-hq-sidecar/`, vorher eigenes Repo mit gepinntem Commit). **Das Quell-Repo ist am 2026-07-30 gelöscht** — der Umzug war ein sauberer Schnitt ohne Historien-Import, die Messbegründungen zu den Encoder-/Puffer-Werten sind vorher nach `docs/2026-07-30-linux-hq-sidecar-messbegruendungen.md` extrahiert worden. Es gibt also **keine ältere Historie mehr** als den Import-Commit; wer eine Zahl hinterfragt, liest das Dokument oder den Code-Kommentar. Flatpak baut ihn per `type: dir` nach `/app/bin/pulse-linux-hq-sidecar`; eine Änderung löst damit **automatisch** einen Flatpak-Build aus. Zwei Dinge bleiben: Dev braucht `$PULSE_LINUX_HQ_SIDECAR` (setzt `dev-up.fish`, wenn `streaming/linux-hq-sidecar/target/release/...` **gebaut** ist — sonst Rückfall auf GSR), und ändert sich `Cargo.lock`, muss `packaging/linux-hq-sidecar-cargo-sources.json` neu generiert werden (Flatpak baut Cargo offline).
...
- **Windows-HQ-Sidecar** (`streaming/win-hq-sidecar/`, Rust): WGC-Capture + wasapi, drei Encode-Pfade (NVENC / AMD-D3D12VA / CPU-Fallback). Voll: `streaming/win-hq-sidecar/README.md` + `WINDOWS_HQ_SIDECAR.md`.
  - **Seit 2026-08-04 kann er Intra-Refresh — und der Weg dorthin ist ein anderer als auf Linux.** Dort genügt die Frage, ob das gelinkte FFmpeg die Option kennt; hier nicht: `h264_d3d12va` (der **Regelweg für H.264 auf AMD**) nimmt sie an und tut nichts damit (0,47 % Bitstrom-Unterschied, kein `constrained_intra_pred_flag`, kein recovery point). Eine Optionstabellen-Abfrage sagte also „ja" und läge falsch. `encode/auffrischung.rs` entscheidet deshalb an einer **Tabelle aus Messungen** je Encoder und **verweigert den Start**, statt still Keyframes unter dem Etikett zu fahren. Getragen wird die Betriebsart auf AMD von **AV1 über `av1_amf`** (`intra_refresh_mode=gop_aligned` + `intra_refresh_stripes`) — die frühere Aussage „AMF kann kein Intra-Refresh" kam davon, dass `-intra_refresh` dort schlicht anders heißt und der Treiber unbekannte Optionsnamen annimmt. Bei `h264_amf` bringt `usage=ultralowlatency` sie ohnehin mit.
  - **Seit 2026-08-06 sendet er HDR** (AV1 10 bit, PQ/BT.2020) — nur er, Linux und macOS können es nicht. Schalter `overrides.hdr`, Fähigkeit `health.gsr.hdr`, Absage-Tabelle je Encoder in `encode/hdr.rs` nach dem Muster von `auffrischung.rs`: **unerfüllbar heißt Startverweigerung**, nicht stiller Rückfall (SDR unter HDR-Etikett sieht man am ganzen Bild, 10 bit weniger als bestellt höchstens an einem Verlauf). HDR schaltet 10 bit selbst ein und ist damit an AV1 gebunden. **Drei nicht offensichtliche Stücke**, alle in `docs/2026-08-06-hdr-windows-amd.md` belegt: (1) Die Aufnahme muss in `Rgba16F`/scRGB laufen — in BGRA ist der Helligkeitsumfang schon weg, und keine spätere Stufe holt ihn zurück. (2) **Der Video-Prozessor kann auf AMD kein PQ** (32 Kombinationen abgefragt, zwei möglich, keine mit PQ; 16-Bit-Fließkomma am Eingang grundsätzlich abgelehnt) — deshalb rechnet ein eigener HLSL-Shader, `encode/hdr_zeichner.rs` (hier stand bis 2026-08-07 `encode/hdr_wandler.rs`; dort liegt seither nur noch der Ziel-Pool), wie GSR es auf Linux tut. **Er läuft seit dem 2026-08-07 schon im Aufnahme-Rückruf** und schreibt direkt aus der WGC-Textur nach P010 (`capture/aufnahmeziel.rs`) — die fp16-Zwischenkopie je Bild entfällt damit, gemessen halbiert das die 3D-Last des Senders (21,2 → 10,6 %; Notausschalter `PULSE_HQ_HDR_ZWISCHENKOPIE=1`, Messakte `streaming/testbench/profiles/leistung-2026-08-07-wandlung-im-rueckruf.json`). Vorbedingung dafür war ein Zähler für WGC-seitig verworfene Bilder (`capture/rueckruf.rs`): WGC meldet solche Verluste nicht, und `capture_drops` kennt nur die eigene Seite. Ziel ist **P010**, obwohl `av1_amf` auch RGBAF16 nähme: `amfenc_av1.c` reicht die Transferkurve nur bei NV12/P010 durch, sonst stünde „Kurve unbekannt" im Sequenzkopf. (3) Die HDR10-Mastering-Metadaten kommen im Strom mit **falschen Zahlen** an (AMF rechnet HDR10-Einheiten nicht in AV1-Festkomma um) — bewusst nicht vorkompensiert, weil ein Treiber-Fix das still falsch machen würde; MaxCLL stimmt, und der eigene Player liest das zuerst.
  - **Nebenbefund derselben Arbeit: der 10-bit-SDR-Weg hat sich als PQ ausgegeben** (`transfer=smpte2084` bei gewöhnlichem SDR, seit 2026-08-04 ausgeliefert). Ursache ist derselbe `amfenc_av1.c`-Zweig, der an `color_primaries != UNSPECIFIED` hängt: wer nur `colorspace` setzt, überspringt ihn, und AMF nimmt für 10 bit von sich aus PQ an. **Nichts zu behaupten genügt nicht — BT.709 muss ausdrücklich dastehen.** Behoben in `encoder_hw.rs`.
  - **Er hat einen eigenen WebRTC-Sendeweg** (`src/whip/`, dieselbe Fassung wie Linux, angemeldet in `main.rs` über `encode::senke`): `http(s)://`-Ziele gehen dorthin statt an ffmpegs WHIP-Muxer, RTMPS bleibt beim Muxer. Grund ist der **Rückkanal** — ein Intra-Refresh-Strom hat nach dem Start kein Vollbild mehr, ohne die RTCP-Anforderung des Zuschauers kommt niemand ins Bild — und **AV1**, das ffmpegs Muxer nicht trägt. Der CPU-Weg (Intel) benutzt weiter den Muxer; folgenlos, weil Intel die Betriebsart ohnehin nicht trägt.
...
**Public well-known-Endpoints** (auth-svc, Root): `/.well-known/{jwks.json, revoked-credentials, pulse-version-policy.json, pulse-suspended-instances}` — Self-Hosts pollen die: `crl_poller.py` (revoked-credentials + jwks, alle 10 s), `cloud_policy_poller.py` (version-policy, alle 6 h), `suspend_poller.py` (suspended-instances, alle 60 s). **Der Sperr-Poller fehlte bis 2026-07-27** — die Liste wurde veröffentlicht und von *niemandem* abgefragt, „Server löschen" war für den Betreiber damit **wirkungslos**: cloud-seitig `status=deleted`, und die Instanz lief unbeirrt weiter (am eigenen Testserver beobachtet). Jetzt verweigert sie Cert-Login (403 `instance_suspended`/`instance_deleted`) und trennt WS-Verbindungen (4003). **Fail-open ist Absicht** — Cloud oder Redis nicht erreichbar heißt „nicht gesperrt", sonst legt ein Cloud-Ausfall jeden Self-Host gleichzeitig lahm. Daten werden NICHT angefasst und der Container NICHT gestoppt (eine Sperre ist umkehrbar; ein Selbstmord-Prozess landet mit `restart: unless-stopped` in der Neustartschleife). **`web-nginx.conf` muss sie explizit an auth-svc routen** (Regex-Location), sonst SPA-Fallback → Poller scheitern still mit JSONDecodeError. `acme-challenge` bewusst ausgespart.
...
**`packaging/build.fish` ERSETZT die installierte App** (endet auf `--user --install`) und hängt die Installation an `.flatpak-builder/cache` um — ab dann zieht `flatpak update` aus dem lokalen Verzeichnis statt vom veröffentlichten Repo, sieht aber wie ein echtes Update aus. Am 2026-07-29 so passiert: ein Verifikations-Build überschrieb die Arbeits-Installation, das nächste `flatpak update` holte den Zwischenstand, weißer Bildschirm. **Nur-Prüfen ohne Installieren:** `flatpak-builder --repo=/tmp/... build/flatpak packaging/com.howispulse.Pulse.yml`, Ergebnis unter `build/flatpak/files/` ansehen. Zurück auf den Auslieferkanal: deinstallieren **ohne** `--delete-data`, dann von `https://howispulse.com/flatpak/com.howispulse.Pulse.flatpakref` installieren; danach muss `flatpak list --columns=application,origin` `pulse-origin` zeigen, nicht `pulse1-origin`.
...
**Hauptserver/Cloud = netcup `michael@159.195.150.54`** (Debian 13), **https://howispulse.com** (Umzug von Hetzner 2026-05-28). Der **alte Hetzner-VPS `michael@77.42.71.166` lebt weiter** als **Self-Host-Test-Instanz** (NICHT mehr Cloud). Ein Compose-Stack (`name: pulse`) in `~/pulse/infra/prod/`: die 6 FastAPI-Dienste (GHCR `ghcr.io/oblivion8282-1337/pulse-*:latest`), `migrate-{auth,chat}`, `mediamtx`+`livekit` (host-net) — **und, oft übersehen, weil hier bis 2026-07-27 nicht genannt: `postgres`, `redis`, `web` (nginx), `minio`+`minio-init` (Uploads), `backup`, `registry` (die Self-Host-Image-Registry) und `frps` (Relay-Server).** Wer nur die erste Hälfte kennt, sucht beim Debuggen Dienste, von deren Existenz er nichts weiß. **Kein Watchtower mehr** (2026-06-11) — Auto-Update läuft über eine **User-Crontab** (`infra/prod/pulse-update.sh`, scoped `compose pull && up -d`); Watchtower mountete den Docker-Socket (= root am Host). Memory `self-host-host-updater`.
...
- **Deploy ist vom Test-Gate ENTKOPPELT** (2026-06-05): der `images`-Job hängt NUR am `changelog`-Job, nicht an backend/frontend (und der `changelog`-Job **warnt nur, blockt nie** — s. Abschnitt „Changelog"; es gibt in der CI also gar kein blockierendes Gate mehr). Grund: seltene pub/sub-Subscribe-Race-Flakes (thread-Timeout, vom `--only-rerun` nicht gefangen) blockten sonst legitime Deploys. backend+frontend bleiben sichtbare Info-Checks. **Konsequenz: das verbindliche Test-Gate ist LOKAL vor dem Push** — pytest + `pnpm check` + build müssen grün sein, BEVOR gepusht wird (kein CI-Netz mehr danach).
...
- **Gotchas**: Secrets nur server-seitig in `.env` + `secrets/jwt_*.pem` — **PEM `chmod 0644`** (Container uid 10001). Avatar-Volume bei Fresh-Deploy `chown 10001:10001` (sonst Upload-500). UFW: `7880`/`9997` nur vom Docker-Bridge (`ufw allow from 10.0.0.0/8`) — sonst blockt `INPUT DROP` den Bridge→Host-Weg. MediaMTX = **1.19.1-pulse2**-Fork (seit 2026-06-16; `infra/mediamtx-fork/`, TempDelim-Patch für AMD-VAAPI AV1, Image `ghcr.io/oblivion8282-1337/pulse-mediamtx:1.19.1-pulse2`) — #5728 ist geschlossen, Bump vollzogen. **1.19 entfernte `apiAllowAddresses`** → der `:9997`-API-Schutz hängt jetzt **NUR noch an der UFW** (vorher doppelt). Dev-compose (`streaming/server/`) läuft **ebenfalls** 1.19.1-pulse2 (Dev und Prod synchron; hier stand bis 2026-07-27 fälschlich „noch 1.17.1", danach bis 2026-08-09 „1.19.1-pulse" ohne das `2`-Patch-Tag). Migrate-Container laufen seit dem Cron-Updater (2026-06-11) automatisch beim Deploy mit (der frühere manuelle `up -d migrate-*`-Schritt entfällt; Memory `watchtower-skips-migrate-containers` für die Historie/Diagnose).
...
- **Code-Simplifier nach jeder Code-Änderung (Regel, NICHT erzwungen):** `code-simplifier`-Agent über die geänderten Dateien → relevante **Tests/Checks erneut grün ziehen** (das ist die Kontrolle, ob die Vereinfachung etwas gebrochen hat — kein zweiter Review-Agent nötig) → `bash .claude/hooks/simplify-stamp.sh` → committen. **Hier stand „Zwei Hooks erzwingen das, beim `git commit` und am Turn-Ende; ohne Stempel blockieren sie" — das stimmt seit dem 2026-06-28 nicht mehr** (Commit `b345ca8d` hat beide aus `.claude/settings.json` genommen, dort stehen nur noch zwei graphify-Hooks). Die Skripte liegen weiter unter `.claude/hooks/` und funktionieren, sie feuern nur niemand mehr. Es ist also eine Disziplinregel; der Stempel bezeugt den Durchlauf, prüft ihn aber nicht nach. **Mechanik und wie man die Gates zurückholt: `.claude/hooks/README.md`.**
- **Eine Behauptung wird nie an nur EINER Stelle korrigiert — vorher greppen.** Wer einen Wert, einen Pfad, einen Optionsnamen, eine Version oder eine Verhaltensaussage ändert, sucht **zuerst**, wo dieselbe Aussage sonst noch steht (`grep -rn "<alter Wert>"`), und zieht alle Fundstellen mit. Die alte Aussage bleibt dabei als widerlegt stehen („hier stand bis … das ist falsch, richtig ist …"), sie verschwindet nicht.

  **Warum das eine eigene Regel ist:** eine Prüfung des Repos am 2026-08-04 (sechs Agenten, je ein Skeptiker dahinter) fand 19 überholte Behauptungen — und fast jede hatte dieselbe Form: *ein* Abschnitt war korrigiert worden, der inhaltsgleiche daneben nicht. Serverumzug Hetzner→netcup, Electron-Pivot, Watchtower-Abschaltung, AMD-Encoder-Wechsel, Abschaltung der Simplifier-Hooks — jedes Mal so. Zwei der Befunde entstanden **am selben Tag, in derselben Stunde**, in der die Regel bekannt war; einer war 29 Minuten nach dem Schreiben überholt. Es ist also keine Frage der Sorgfalt, sondern eine, die einen Handgriff braucht.

  Besonders anfällig: IP-Adressen und Hostnamen, Versionsnummern (Electron, MediaMTX, FFmpeg), Env-Variablennamen, Encoder- und Optionsnamen, Ops-Listen im Protokoll, Lizenzbezeichnungen, und alles, was gleichzeitig in `CLAUDE.md`, einer `README.md` und einem Code-Kommentar steht.
...
**Redaktionelle Regel, KEIN technisches Gate** (korrigiert 2026-07-27): `scripts/check-changelog.sh` (Job `changelog` in `ci.yml`) **warnt nur** — seit 2026-06-28 endet es in jedem Pfad mit `exit 0`, damit Hotfixes nicht blockieren. `images` hängt zwar an dem Job, aber der schlägt nie fehl → **der Deploy läuft in jedem Fall, auch ohne Eintrag.** Ein fehlender Eintrag ist Nachlässigkeit, kein Fehler. Der frühere Wortlaut hier („sonst blockt das CI-Gate") war falsch und hat mehrfach zu unnötigen Einträgen geführt.

Wann ein Eintrag wirklich sinnvoll ist: wenn ein Nutzer die Änderung **bemerken** würde. Verfeinert ein Commit nur etwas, das ein Eintrag desselben Tages schon angekündigt hat, gehört die Ergänzung in **diesen** Eintrag — ein zweiter Toast am selben Tag zum selben Thema ist Lärm.

**Workflow vor einem Push auf main:**
...
2. **Stil NICHT selbst wählen** — dem User **mehrere Stil-Vorschläge** machen, er entscheidet (zuletzt durchgehend „Sachlich"). **KEINE Emojis — nirgends** (nicht im Titel, Intro, Items oder Outro; gilt auch für Assistenten-Antworten). User-Wunsch 2026-06-08, harte Regel. **Eine bewusste Ausnahme steht im Baum:** der Eintrag `2026-08-05` trägt eine Rakete im Titel, vom User am 2026-08-05 ausdrücklich für diesen einen Eintrag freigegeben („da darf auch mal ein emoji hin ausnahmsweise"). **Nicht wegräumen** — und daraus auch keine neue Gewohnheit machen: für jeden weiteren Eintrag gilt die Regel unverändert, bis der User erneut etwas anderes sagt. **Echte Umlaute (ä/ö/ü/ß)** für neue Changelog-Einträge **und** Commit-Messages — nicht die Transliteration ae/oe/ue/ss, die hier früher nur Gewohnheit war (User-Wunsch 2026-08-09). Vorsicht bei ss→ß, das mehrdeutig ist („dass" bleibt, „Straße" wird zu „ß") — Wort für Wort, kein Blind-Replace. Die 260+ älteren Changelog-Einträge stehen weiter in ae/oe/ue (Archiv: jeder Eintrag wird nur einmal bei seiner ID gezeigt, nie wieder — eine Voll-Konvertierung nur auf ausdrücklichen Wunsch).
3. Neuen Eintrag oben in `entries` einfügen, `id` eindeutig (Datum; mehrere/Tag → `2026-06-05.2`).
4. Nur **user-facing** Code verlangt einen Eintrag. Nicht-user-facing Pfade sind ausgenommen — **maßgeblich ist `NON_USER_FACING` in `scripts/check-changelog.sh`, nicht diese Aufzählung**: `*.md`, `LICENSE*`, `docs/`, `.github/`, `infra/`, `packaging/`, `scripts/`, `Dockerfile*`, `*.toml`, `*.yaml`/`*.yml`, `build-resources/`, `package.json`, `*/tests/`, `conftest.py`, `*/alembic/`, `web/static/install.sh`, `.*ignore`, die `changelog.json` selbst. (Bis 2026-08-04 fehlten hier sieben davon — die Liste gab sich als vollständig aus und war es nie.)
